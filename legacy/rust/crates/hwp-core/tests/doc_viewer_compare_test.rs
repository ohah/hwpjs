...
#[test]
fn cross_compare_hwp_vs_hwpx_html() {
    let hwp_files = common::find_all_hwp_files();
    let parser = HwpParser::new();
    let options = DocHtmlOptions::default();
    ...
    let mut hwpx_missing = 0;

    for hwp_path in &hwp_files {
        let hwp_path = std::path::Path::new(hwp_path);
        let stem = hwp_path.file_stem().unwrap().to_str().unwrap();
        let hwpx_path = hwp_path.with_extension("hwpx");
        ...
        let hwp_data = std::fs::read(hwp_path).unwrap();
        let hwpx_data = std::fs::read(&hwpx_path).unwrap();

        let hwp_doc = match parser.parse(&hwp_data) {
            Ok(d) => d,
            Err(_) => continue,
        };
        let hwp_document = to_document(&hwp_doc);

        let hwpx_document = match hwpx_parser::HwpxParser::parse(&hwpx_data) {
            Ok(d) => d,
            Err(_) => continue,
        };

        let hwp_html = doc_to_html(&hwp_document, &options);
        let hwpx_html = doc_to_html(&hwpx_document, &options);

        if hwp_html == hwpx_html {
            matched += 1;
        } else {
            let size_diff = (hwpx_html.len() as i64) - (hwp_html.len() as i64);
            println!(
                "  DIFF {:40} HWP:{:>7}B HWPX:{:>7}B ({:+})",
                stem, hwp_html.len(), hwpx_html.len(), size_diff,
            );
            diffed.push(stem.to_string());
        }
    }

    println!(
        "\n=== HWP↔HWPX CROSS: {} matched, {} diffed, {} missing ===",
        matched, diffed.len(), hwpx_missing
    );
    if !diffed.is_empty() {
