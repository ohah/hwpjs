    ...
    program
        ...
        try {
            ...
            if (!options.includeImages && result.images.length > 0 && options.output) {
                ...
                const outputDir = (0, path_1.dirname)((0, path_1.resolve)(options.output));
                const imagesDir = options.imagesDir
                    ? (0, path_1.resolve)(outputDir, options.imagesDir)
                    : (0, path_1.join)(outputDir, 'images');
                ...
                for (const image of result.images) {
                    ...
                    const imagePath = (0, path_1.join)(imagesDir, fileName);
                    (0, fs_1.writeFileSync)(imagePath, image.data);
                    imagesSaved++;
                    ...
                    const relativePath = options.imagesDir
                        ? (0, path_1.join)(options.imagesDir, fileName).replace(/\\/g, '/')
                        : (0, path_1.join)('images', fileName).replace(/\\/g, '/');
            ...
            // Write output
            if (options.output) {
                (0, fs_1.writeFileSync)(options.output, finalMarkdown, 'utf-8');
