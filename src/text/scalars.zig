...
/// Strict Unicode scalar decoding, without document-specific character policy.
pub fn read(bytes: []const u8, offset: usize, encoding: Encoding) !?Scalar {
    ...
    if (encoding == .utf8) {
        const n = std.unicode.utf8ByteSequenceLength(rest[0]) catch return error.InvalidUnicodeEncoding;
        if (n > rest.len) return error.UnexpectedEnd;
        const c = std.unicode.utf8Decode(rest[0..n]) catch return error.InvalidUnicodeEncoding;
    ...
    if (first >= 0xdc00 and first <= 0xdfff) return error.InvalidUnicodeEncoding;
    ...
    if (second < 0xdc00 or second > 0xdfff) return error.InvalidUnicodeEncoding;
