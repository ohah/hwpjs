...
pub fn materializeGridCellNumber(a: std.mem.Allocator, hwp: []const u8, ordinal: usize, storage_layout: StorageLayout, ole_layout: @import("../ole/envelope.zig").Layout, chart_layout: ChartLayout, row: usize, column: usize, bits: u64, trailer: u16, options: Options) ![]u8 {
...
fn editContents(a: std.mem.Allocator, source: []const u8, chart_layout: ChartLayout, commands: []const Edit, max_output_bytes: usize, options: Options) ![]u8 {
    ...
    for (order, 0..) |i, request_at| {
        ...
        if (resolved[i] == .null_grid_number) {
            const target = resolved[i].null_grid_number;
            const object_id = try @import("../chart/object_id_allocator.zig").findLowestAvailable(&chart.prefix.objects, forbidden[0..reserved]);
            forbidden[reserved] = object_id;
            reserved += 1;
            const number_type_id = try findLowestTypeIdAvoiding(&chart.prefix.grid.prelude.types, requests[0..request_at], &.{});
            const value_type_id = try findLowestTypeIdAvoiding(&chart.prefix.grid.prelude.types, requests[0..request_at], &.{number_type_id});
...
fn findLowestTypeIdAvoiding(types: *const @import("../chart/type_table.zig").Table, previous: []const @import("../chart/contents_string_fork.zig").BatchRequest, extra: []const u32) !u32 {
    ...
    while (candidate != 0xffffffff) : (candidate += 1) {
        ...
        for (previous) |request| switch (request) {
            ...
            .null_grid_number => |item| blocked = blocked or item.number_type_id == candidate or item.value_type_id == candidate,
            else => {},
        };
