...
pub const Identity = enum { exact, observed_memo, observed_revision_delete };
/// Observed memo/deletion representations, not a general unknown-field wildcard.
...
pub fn resolve(token_id: u32, header_id: u32, code: u16, properties: []const u8) !Identity {
    ...
    if ((token_id == rules.id("%%me") or token_id == rules.id("%%*d")) and header_id == rules.id("%unk") and code == 3) {
