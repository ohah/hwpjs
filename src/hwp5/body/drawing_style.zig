...
pub const TailLayout = enum { fill_only, alpha_shadow, alpha_shadow_metadata };
pub const Tail = union(enum) {
    /// Explicit older layout: no alpha/shadow interpretation; preserve remaining bytes.
    fill_only: []const u8,
    known: struct { alpha: Alpha, shadow: Shadow, metadata: ?Metadata, extra: []const u8 },
    unknown: []const u8,
};
    ...
    pub fn parseWithTail(bytes: []const u8, layout: border.Layout, tail_layout: TailLayout) !Style {
        ...
        const tail: Tail = switch (fill.data) {
            ...
            .known => |known| blk: {
                ...
                const metadata = if (tail_layout == .alpha_shadow_metadata) try Metadata.read(&rest) else null;
                break :blk .{ .known = .{ .alpha = alpha, .shadow = shadow, .metadata = metadata, .extra = known.extra[rest.offset..] } };
            },
        };
