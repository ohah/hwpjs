...
const note_text = @import("note_text.zig");
...
pub const NoteTextOptions = struct { trees: XmlTreesOptions = .{}, notes: note_bodies.Options = .{}, text: note_text.Options = .{} };
pub const NoteTextReport = note_text.Report;
...
pub const LineSegmentsOptions = line_segments.Options;
pub const LineSegmentsReport = line_segments.Report;
...
pub const SectionNoteShapeOptions = section_note_shapes.Options;
...
pub const FillBrushImageLinkOptions = fill_brush_image_links.Options;
...
pub const Document = struct {
    ...
    /// Owns direct text and exact XML for section hp:metaTag elements without
    /// interpreting application metadata or rendering behavior.
    pub fn inspectMetaTags(self: *const Document, a: std.mem.Allocator, options: MetaTagOptions) !MetaTagReport {
        var trees = try self.readXmlTrees(a, options.trees);
        defer trees.deinit(a);
    ...
    pub fn inspectNoteText(self: *const Document, a: std.mem.Allocator, options: NoteTextOptions) !NoteTextReport {
        var trees = try self.readXmlTrees(a, options.trees);
        defer trees.deinit(a);
        var notes = try trees.inspectNoteBodies(a, options.notes);
        defer notes.deinit();
        return trees.inspectNoteText(a, &notes, options.text);
    }
