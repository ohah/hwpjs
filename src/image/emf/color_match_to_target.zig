...
pub fn parse(record: records.Record) !?ColorMatchToTarget {
    ...
    _ = record_extent.requiredEnd(record, fixed_size) orelse return error.InvalidEmfColorMatchToTargetSize;
    ...
    return .{
        ...
        .name_utf16le = name,
        .profile_data = record.bytes[name_end..data_end],
        .trailing_data = record.bytes[data_end..],
...
test "COLORMATCHTOTARGETW parses empty and populated borrowed payloads" {
    ...
    const disabled = (try parse(fixture(.colormatchtotargetw, &empty))) orelse return error.TestExpectedEqual;
    ...
    try std.testing.expectEqual(@as(usize, 0), disabled.trailing_data.len);
    ...
    bytes[32..36].* = .{ 1, 2, 3, 4 };
    bytes[36..40].* = .{ 0xde, 0xad, 0xbe, 0xef };
    const value = (try parse(fixture(.colormatchtotargetw, &bytes))) orelse return error.TestExpectedEqual;
    ...
    try std.testing.expectEqualSlices(u8, bytes[36..40], value.trailing_data);
    try std.testing.expectEqual(@as(usize, 3), value.name_stats.scalars);
    try std.testing.expect(value.name_stats.ends_in_nul);
}

test "COLORMATCHTOTARGETW validates payload extents values UTF16 and identity" {
    var bytes = [_]u8{0} ** 32;
    ...
    const shortened = (try parse(fixture(.colormatchtotargetw, &short_name))) orelse return error.TestExpectedEqual;
    try std.testing.expectEqualSlices(u8, short_name[24..26], shortened.name_utf16le);
    try std.testing.expectEqualSlices(u8, short_name[26..30], shortened.profile_data);
    try std.testing.expectEqualSlices(u8, short_name[30..32], shortened.trailing_data);
    var huge = bytes;
