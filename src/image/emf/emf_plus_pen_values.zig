...
pub const UnitType = enum(u32) { world = 0, display = 1, pixel = 2, point = 3, inch = 4, document = 5, millimeter = 6 };
...
pub fn unitType(raw: u32) !UnitType {
    return switch (raw) {
        0 => .world,
        1 => .display,
        2 => .pixel,
...
test "EMF+ pen value domains reject gaps and reserved flag bits" {
    ...
    try std.testing.expectEqual(UnitType.millimeter, try unitType(6));
    try std.testing.expectError(error.InvalidEmfPlusUnitType, unitType(7));
