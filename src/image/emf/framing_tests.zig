...
test "EMF framing routes SetTSGraphics through the EMF+ stream" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4039, .little);
    std.mem.writeInt(u32, bytes[136..140], 48, .little);
    std.mem.writeInt(u32, bytes[140..144], 36, .little);
    bytes[147] = 1;
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.set_ts_graphics_records);

    var malformed = bytes;
    malformed[154] = 5;
    try t.expectError(error.InvalidEmfPlusFilterType, framing.validate(t.allocator, &malformed));
}

test "EMF framing routes MultiplyWorldTransform through the EMF+ stream" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x402c, .little);
    std.mem.writeInt(u16, bytes[134..136], 0x2000, .little);
    std.mem.writeInt(u32, bytes[136..140], 36, .little);
    std.mem.writeInt(u32, bytes[140..144], 24, .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.multiply_world_transform_records);
}

test "EMF framing routes SetWorldTransform through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 200;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x402a, .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.set_world_transform_records);
}

test "EMF framing routes ResetWorldTransform through the EMF+ stream" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x402b, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xffff, .little);
    std.mem.writeInt(u32, bytes[136..140], 12, .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.reset_world_transform_records);
}

test "EMF framing routes TranslateWorldTransform through the EMF+ stream" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x402d, .little);
    std.mem.writeInt(u16, bytes[134..136], 0x2000, .little);
    std.mem.writeInt(u32, bytes[136..140], 20, .little);
    std.mem.writeInt(u32, bytes[140..144], 8, .little);
    std.mem.writeInt(u32, bytes[144..148], @bitCast(@as(f32, -1.5)), .little);
    std.mem.writeInt(u32, bytes[148..152], @bitCast(@as(f32, 2.25)), .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.translate_world_transform_records);
}

test "EMF framing routes ScaleWorldTransform through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 184;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x402e, .little);
    std.mem.writeInt(u16, bytes[134..136], 0x2000, .little);
    std.mem.writeInt(u32, bytes[136..140], 20, .little);
    std.mem.writeInt(u32, bytes[140..144], 8, .little);
    std.mem.writeInt(u32, bytes[144..148], @bitCast(@as(f32, -1.5)), .little);
...
test "EMF framing routes SetPageTransform through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 180;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4030, .little);
    std.mem.writeInt(u16, bytes[134..136], 0, .little);
    std.mem.writeInt(u32, bytes[136..140], 16, .little);
    std.mem.writeInt(u32, bytes[140..144], 4, .little);
    std.mem.writeInt(u32, bytes[144..148], @bitCast(@as(f32, 1.5)), .little);
    std.mem.writeInt(u16, bytes[148..150], 0x4002, .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.set_page_transform_records);
    try t.expectEqual(@as(usize, 1), report.set_page_transform_discouraged_page_unit_records);
}

test "EMF framing routes ResetClip through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 176;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4031, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xffff, .little);
    std.mem.writeInt(u32, bytes[136..140], 12, .little);
    std.mem.writeInt(u16, bytes[144..146], 0x4002, .little);
    std.mem.writeInt(u32, bytes[148..152], 12, .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.reset_clip_records);
}

test "EMF framing routes SetClipRect through the EMF+ stream" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4032, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xf5ff, .little);
    std.mem.writeInt(u32, bytes[136..140], 28, .little);
    std.mem.writeInt(u32, bytes[140..144], 16, .little);
    std.mem.writeInt(u32, bytes[144..148], @bitCast(@as(f32, -1.5)), .little);
    std.mem.writeInt(u32, bytes[148..152], @bitCast(@as(f32, 2.5)), .little);
    std.mem.writeInt(u32, bytes[152..156], @bitCast(@as(f32, 3.5)), .little);
    std.mem.writeInt(u32, bytes[156..160], @bitCast(@as(f32, 4.5)), .little);
    ...
    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.set_clip_rect_records);
}

test "EMF framing routes SetClipPath through the EMF+ Object Table" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[96..100], 68, .little);
    std.mem.writeInt(u32, bytes[100..104], 0x2b464d45, .little);
    std.mem.writeInt(u16, bytes[104..106], 0x4001, .little);
    std.mem.writeInt(u32, bytes[108..112], 28, .little);
    std.mem.writeInt(u32, bytes[112..116], 16, .little);
    ...
    std.mem.writeInt(u32, bytes[136..140], 12, .little);
    std.mem.writeInt(u16, bytes[144..146], 0x4033, .little);
    std.mem.writeInt(u16, bytes[146..148], 0xf505, .little);
    std.mem.writeInt(u32, bytes[148..152], 12, .little);
    std.mem.writeInt(u16, bytes[156..158], 0x4002, .little);
    std.mem.writeInt(u32, bytes[160..164], 12, .little);
    @memcpy(bytes[168..188], original[88..108]);

    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.set_clip_path_records);
    var missing = bytes;
    std.mem.writeInt(u16, missing[146..148], 0xf506, .little);
    try t.expectError(error.MissingEmfPlusSetClipPathPath, framing.validate(t.allocator, &missing));
}

test "EMF framing routes SetClipRegion through the EMF+ Object Table" {
    const original = fixture();
    var bytes = [_]u8{0} ** 188;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4008, .little);
    std.mem.writeInt(u16, bytes[134..136], 0x0405, .little);
    std.mem.writeInt(u32, bytes[136..140], 12, .little);
    std.mem.writeInt(u16, bytes[144..146], 0x4034, .little);
    std.mem.writeInt(u16, bytes[146..148], 0xf505, .little);
    std.mem.writeInt(u32, bytes[148..152], 12, .little);
    std.mem.writeInt(u16, bytes[156..158], 0x4002, .little);
    std.mem.writeInt(u32, bytes[160..164], 12, .little);
    ...
    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.set_clip_region_records);
    var missing = bytes;
    std.mem.writeInt(u16, missing[146..148], 0xf506, .little);
    try t.expectError(error.MissingEmfPlusSetClipRegionRegion, framing.validate(t.allocator, &missing));
}

test "EMF framing routes OffsetClip through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 184;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4035, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xffff, .little);
    std.mem.writeInt(u32, bytes[136..140], 20, .little);
    std.mem.writeInt(u32, bytes[140..144], 8, .little);
    std.mem.writeInt(u32, bytes[144..148], @bitCast(@as(f32, -1.5)), .little);
    std.mem.writeInt(u32, bytes[148..152], 0x7fc00001, .little);
    std.mem.writeInt(u16, bytes[152..154], 0x4002, .little);
    std.mem.writeInt(u32, bytes[156..160], 12, .little);
    @memcpy(bytes[164..184], original[88..108]);

    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.offset_clip_records);
}

test "EMF framing reports opaque StrokeFillPath without inferring payload fields" {
    const original = fixture();
    var bytes = [_]u8{0} ** 180;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4037, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xa55a, .little);
    std.mem.writeInt(u32, bytes[136..140], 16, .little);
    std.mem.writeInt(u32, bytes[140..144], 4, .little);
    bytes[144..148].* = .{ 0x00, 0x7f, 0x80, 0xff };
    std.mem.writeInt(u16, bytes[148..150], 0x4002, .little);
    std.mem.writeInt(u32, bytes[152..156], 12, .little);
    @memcpy(bytes[160..180], original[88..108]);

    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.opaque_stroke_fill_path_records);
}

test "EMF framing routes literal-color FillRects through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 192;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x400a, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xc000, .little);
    std.mem.writeInt(u32, bytes[136..140], 28, .little);
    std.mem.writeInt(u32, bytes[140..144], 16, .little);
    std.mem.writeInt(u32, bytes[144..148], 0x44332211, .little);
    std.mem.writeInt(u32, bytes[148..152], 1, .little);
    for (0..4) |index| std.mem.writeInt(i16, bytes[152 + index * 2 ..][0..2], @intCast(index), .little);
    std.mem.writeInt(u16, bytes[160..162], 0x4002, .little);
    std.mem.writeInt(u32, bytes[164..168], 12, .little);
    @memcpy(bytes[172..192], original[88..108]);

    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.fill_rects_records);
}

test "EMF framing routes literal-color FillPolygon through the EMF+ stream" {
    const original = fixture();
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x400c, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xc000, .little);
    std.mem.writeInt(u32, bytes[136..140], 32, .little);
    std.mem.writeInt(u32, bytes[140..144], 20, .little);
    std.mem.writeInt(u32, bytes[144..148], 0x44332211, .little);
    std.mem.writeInt(u32, bytes[148..152], 3, .little);
    for (0..6) |index| std.mem.writeInt(i16, bytes[152 + index * 2 ..][0..2], @intCast(index), .little);
    ...
    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.fill_polygon_records);
}

test "EMF framing routes literal-color FillEllipse through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 188;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x400e, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xc000, .little);
    std.mem.writeInt(u32, bytes[136..140], 24, .little);
    std.mem.writeInt(u32, bytes[140..144], 12, .little);
    std.mem.writeInt(u32, bytes[144..148], 0x44332211, .little);
    for (0..4) |index| std.mem.writeInt(i16, bytes[148 + index * 2 ..][0..2], @intCast(index), .little);
    std.mem.writeInt(u16, bytes[156..158], 0x4002, .little);
    std.mem.writeInt(u32, bytes[160..164], 12, .little);
    @memcpy(bytes[168..188], original[88..108]);
...
test "EMF framing routes literal-color FillRegion through its Region reference" {
    const original = fixture();
    var bytes = [_]u8{0} ** 192;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u16, bytes[134..136], 0x0405, .little);
    std.mem.writeInt(u32, bytes[136..140], 12, .little);
    std.mem.writeInt(u16, bytes[144..146], 0x4013, .little);
...
test "EMF framing routes literal-color FillClosedCurve through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 200;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4016, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xe000, .little);
    std.mem.writeInt(u32, bytes[136..140], 36, .little);
    std.mem.writeInt(u32, bytes[140..144], 24, .little);
    std.mem.writeInt(u32, bytes[144..148], 0x44332211, .little);
    std.mem.writeInt(u32, bytes[148..152], @bitCast(@as(f32, 0.5)), .little);
    std.mem.writeInt(u32, bytes[152..156], 3, .little);
    for (0..6) |index| std.mem.writeInt(i16, bytes[156 + index * 2 ..][0..2], @intCast(index), .little);
    std.mem.writeInt(u16, bytes[168..170], 0x4002, .little);
    std.mem.writeInt(u32, bytes[172..176], 12, .little);
    @memcpy(bytes[180..200], original[88..108]);

    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.fill_closed_curve_records);
}
...
test "EMF framing routes SetCompositingMode through the EMF+ stream" {
    const original = fixture();
    var bytes = [_]u8{0} ** 176;
    @memcpy(bytes[0..88], original[0..88]);
    std.mem.writeInt(u32, bytes[48..52], bytes.len, .little);
    ...
    std.mem.writeInt(u32, bytes[124..128], 96, .little);
    std.mem.writeInt(u32, bytes[128..132], 96, .little);
    std.mem.writeInt(u16, bytes[132..134], 0x4023, .little);
    std.mem.writeInt(u16, bytes[134..136], 0xff01, .little);
    std.mem.writeInt(u32, bytes[136..140], 12, .little);
    std.mem.writeInt(u16, bytes[144..146], 0x4002, .little);
    std.mem.writeInt(u32, bytes[148..152], 12, .little);
    @memcpy(bytes[156..176], original[88..108]);

    try t.expectEqual(@as(usize, 1), (try framing.validate(t.allocator, &bytes)).emf_plus.set_compositing_mode_records);
    var invalid = bytes;
    std.mem.writeInt(u16, invalid[134..136], 0xff02, .little);
...
test "EMF framing routes BeginContainer through the shared graphics state stack" {
    ...
    std.mem.writeInt(u16, bytes[148..150], 0x4027, .little);
    std.mem.writeInt(u16, bytes[150..152], 2, .little);
    std.mem.writeInt(u32, bytes[152..156], 48, .little);
    std.mem.writeInt(u32, bytes[156..160], 36, .little);
    for (0..8) |index|
        std.mem.writeInt(u32, bytes[160 + index * 4 ..][0..4], @bitCast(@as(f32, @floatFromInt(index + 1))), .little);
    std.mem.writeInt(u32, bytes[192..196], 20, .little);
    std.mem.writeInt(u16, bytes[196..198], 0x4026, .little);
    std.mem.writeInt(u32, bytes[200..204], 16, .little);
    std.mem.writeInt(u32, bytes[204..208], 4, .little);
    std.mem.writeInt(u32, bytes[208..212], 10, .little);
    std.mem.writeInt(u16, bytes[212..214], 0x4002, .little);
    std.mem.writeInt(u32, bytes[216..220], 12, .little);
    @memcpy(bytes[224..244], original[88..108]);

    const report = (try framing.validate(t.allocator, &bytes)).emf_plus;
    try t.expectEqual(@as(usize, 1), report.begin_container_records);
    try t.expectEqual(@as(usize, 0), report.begin_container_discouraged_page_unit_records);
    try t.expectEqual(@as(usize, 2), report.graphics_state_max_depth);

    var unclosed = bytes;
    std.mem.writeInt(u16, unclosed[196..198], 0x4025, .little);
    try t.expectError(error.UnclosedEmfPlusGraphicsStateStack, framing.validate(t.allocator, &unclosed));

    var malformed = bytes;
    std.mem.writeInt(u16, malformed[150..152], 0x0102, .little);
