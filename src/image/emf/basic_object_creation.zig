...
pub fn parse(record: records.Record) !?Creation {
    return switch (record.kind) {
        .createpen => blk: {
            if (!record_extent.hasRequiredPrefix(record, 28)) return error.InvalidEmfCreatePenRecordSize;
            break :blk .{ .pen = .{
        ...
        .createbrushindirect => blk: {
            if (!record_extent.hasRequiredPrefix(record, 24)) return error.InvalidEmfCreateBrushIndirectRecordSize;
            break :blk .{ .brush = .{
                .handle = std.mem.readInt(u32, record.bytes[8..12], .little),
...
test "basic object creation requires prefixes and accepts trailing data" {
    var pen = [_]u8{0} ** 29;
    ...
    try std.testing.expect((try parse(fixture(.createpen, &pen))) != null);
    ...
    try std.testing.expect((try parse(fixture(.createbrushindirect, &brush))) != null);
