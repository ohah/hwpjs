...
const narrow_comparison = @import("power_root_compare.zig");
...
pub fn locate(comptime precision: u16, root: Root, a: i32, b: i32, interval: Interval) !Location {
    return With(Root, narrow_comparison).locate(precision, root, a, b, interval);
}
/// Share interval ownership and uncertainty rules across root representations.
pub fn With(comptime R: type, comptime comparison: type) type {
    return struct {
        pub fn locate(comptime precision: u16, root: R, a: i32, b: i32, interval: Interval) !Location {
