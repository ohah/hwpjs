...
const solve = @import("parametric_preimage.zig").solve;
test "whole preimages preserve a jump gap and the lower open endpoint" {
    ...
    try std.testing.expectEqual(@as(u256, 0), lower.start.numerator);
    try std.testing.expectEqual(@as(u256, 32768), lower.end.numerator);
    try std.testing.expectEqual(@as(u256, 65536), lower.end.denominator);
...
test "whole preimages preserve the terminal singleton and inactive power branch" {
...
test "whole preimages retain three disconnected roots without imposing invertibility" {
    const curve = Curve{ .function = .type4, .values = .{ 131072, 262144, -196608, 65536, 32768, 0, 0 } };
    const result = (try solve(512, curve, 1, 4)).set;
...
test "whole preimages hide an exact lower solution if the upper result is undecided" {
    ...
    const resolved = (try solve(512, curve, n, d)).set;
...
test "whole preimages validate the entire domain before exposing lower solutions" {
