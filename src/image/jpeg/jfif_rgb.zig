...
pub const Options = struct {
    ...
    component_ids: @import("jfif.zig").ComponentIds = .strict,
};
...
pub fn decode(a: std.mem.Allocator, bytes: []const u8, options: Options) !Image {
    var prepared = try render.Prepared.init(a, bytes, options.planes.frame.structure, .{ .upsampling = options.upsampling, .colour_management = options.colour_management, .max_rgb_bytes = options.max_rgb_bytes, .max_adobe_markers = options.max_adobe_markers, .max_icc_bytes = options.max_icc_bytes, .component_ids = options.component_ids });
