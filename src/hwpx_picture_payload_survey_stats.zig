...
pub const Group = struct {
    ...
    targets: usize = 0,
    formats: [9]usize = @splat(0),
    mismatches: usize = 0,
    failures: usize = 0,
    png_failures: usize = 0,
    wmf_failures: usize = 0,
    tiff_failures: usize = 0,
    pcx_failures: usize = 0,
    svg_failures: usize = 0,
    encoded_bytes: usize = 0,

    fn merge(self: *Group, other: Group) void {
        inline for (.{ "sites", "non_embedded", "targets", "mismatches", "failures", "png_failures", "wmf_failures", "tiff_failures", "pcx_failures", "svg_failures", "encoded_bytes" }) |field| @field(self, field) += @field(other, field);
...
fn check(items: manifest.Manifest, linked: *const links.Report, report: *const payloads.Report) !Group {
    ...
    for (report.targets) |target| {
        ...
        if (target.format == .pcx and target.inspection_error != null) result.pcx_failures += 1;
        if (target.format == .svg and target.inspection_error != null) result.svg_failures += 1;
