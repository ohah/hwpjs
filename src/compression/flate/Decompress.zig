// Adapted from Zig 0.16.0; Copyright (c) Zig contributors, MIT.
// See licenses/Zig-MIT.txt and THIRD_PARTY_NOTICES.md for local changes.
...
fn tossBitsShort(d: *Decompress, n: u4) !void {
    // Local fix: consumed bits are unavailable, not additional capacity.
...
test "short-bit EOF excludes consumed bits and leaves input unchanged" {
    var input: Reader = .fixed(&.{0});
    var decoder: Decompress = .init(&input, .raw, &.{});
    decoder.consumed_bits = 7;
    try testing.expectError(error.EndOfStream, decoder.tossBitsShort(2));
    try testing.expectEqual(@as(usize, 0), input.seek);
    try testing.expectEqual(@as(u3, 7), decoder.consumed_bits);
    try decoder.tossBitsShort(1);
    try testing.expectEqual(@as(usize, 1), input.seek);
}
