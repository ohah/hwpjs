...
- [저작권](#저작권) - 페이지 1
- [본 문서에 대하여](#본-문서에-대하여) - 페이지 2
- [I. 한글 5.0 파일 구조](#i-한글-50-파일-구조) - 페이지 3
  - [1. 개요](#1-개요) - 페이지 5
  - [2. 자료형 설명](#2-자료형-설명) - 페이지 6
  - [3. 한글 파일 구조](#3-한글-파일-구조) - 페이지 7
    - [3.1. 한글 파일 구조 요약](#31-한글-파일-구조-요약) - 페이지 7
    - [3.2. 스토리지 별 저장 정보](#32-스토리지-별-저장-정보) - 페이지 7
      - [3.2.1. 파일 인식 정보](#321-파일-인식-정보) - 페이지 7
      - [3.2.2. 문서 정보](#322-문서-정보) - 페이지 8
      - [3.2.3. 본문](#323-본문) - 페이지 9
      - [3.2.4. 문서 요약](#324-문서-요약) - 페이지 12
      - [3.2.5. 바이너리 데이터](#325-바이너리-데이터) - 페이지 12
      - [3.2.6. 미리보기 텍스트](#326-미리보기-텍스트) - 페이지 12
      - [3.2.7. 미리보기 이미지](#327-미리보기-이미지) - 페이지 13
      - [3.2.8. 문서 옵션](#328-문서-옵션) - 페이지 13
      - [3.2.9. 스크립트](#329-스크립트) - 페이지 13
      - [3.2.10. XML 템플릿](#3210-xml-템플릿) - 페이지 13
      - [3.2.11. 문서 이력 관리](#3211-문서-이력-관리) - 페이지 14
      - [3.2.12. 참고문헌](#3212-참고문헌) - 페이지 15
  - [4. 데이터 레코드](#4-데이터-레코드) - 페이지 16
    - [4.1. 데이터 레코드 구조](#41-데이터-레코드-구조) - 페이지 16
    - [4.2. '문서 정보'의 데이터 레코드](#42-문서-정보의-데이터-레코드) - 페이지 17
      - [4.2.1. 문서 속성](#421-문서-속성) - 페이지 17
      - [4.2.2. 아이디 매핑 헤더](#422-아이디-매핑-헤더) - 페이지 17
      - [4.2.3. 바이너리 데이터](#423-바이너리-데이터) - 페이지 18
      - [4.2.4. 글꼴](#424-글꼴) - 페이지 20
      - [4.2.5. 테두리/배경](#425-테두리배경) - 페이지 21
      - [4.2.6. 글자 모양](#426-글자-모양) - 페이지 24
      - [4.2.7. 탭 정의](#427-탭-정의) - 페이지 26
      - [4.2.8. 문단 번호](#428-문단-번호) - 페이지 26
      - [4.2.9. 글머리표](#429-글머리표) - 페이지 27
      - [4.2.10. 문단 모양](#4210-문단-모양) - 페이지 28
      - [4.2.11. 스타일(문단 스타일)](#4211-스타일문단-스타일) - 페이지 29
      - [4.2.12. 문서 임의의 데이터](#4212-문서-임의의-데이터) - 페이지 30
      - [4.2.13. 배포용 문서 데이터](#4213-배포용-문서-데이터) - 페이지 31
      - [4.2.14. 호환 문서](#4214-호환-문서) - 페이지 32
      - [4.2.15. 레이아웃 호환성](#4215-레이아웃-호환성) - 페이지 32
    - [4.3. '본문'의 데이터 레코드](#43-본문의-데이터-레코드) - 페이지 33
      - [4.3.1. 문단 헤더 Tag ID : HWPTAG_PARA_HEADER](#431-문단-헤더-tag-id--hwptag_para_header) - 페이지 33
      - [4.3.2. 문단의 텍스트](#432-문단의-텍스트) - 페이지 34
      - [4.3.3. 문단의 글자 모양](#433-문단의-글자-모양) - 페이지 34
      - [4.3.4. 문단의 레이아웃](#434-문단의-레이아웃) - 페이지 35
      - [4.3.5. 문단의 영역 태그](#435-문단의-영역-태그) - 페이지 35
      - [4.3.6. 컨트롤 헤더](#436-컨트롤-헤더) - 페이지 35
      - [4.3.7. 문단 리스트 헤더](#437-문단-리스트-헤더) - 페이지 36
      - [4.3.8. 컨트롤 임의의 데이터](#438-컨트롤-임의의-데이터) - 페이지 36
      - [4.3.9. 개체 공통 속성을 포함하는 컨트롤(개체 컨트롤)](#439-개체-공통-속성을-포함하는-컨트롤개체-컨트롤) - 페이지 36
        - [4.3.9.1. 표 개체](#4391-표-개체) - 페이지 39
        - [4.3.9.2. 그리기 개체(선, 사각형, 타원, 호, 다각형, 곡선)](#4392-그리기-개체선-사각형-타원-호-다각형-곡선) - 페이지 40
          - [4.3.9.2.1. 개체 요소](#43921-개체-요소) - 페이지 40
          - [4.3.9.2.2. 선 개체](#43922-선-개체) - 페이지 42
          - [4.3.9.2.3. 사각형 개체](#43923-사각형-개체) - 페이지 43
          - [4.3.9.2.4. 타원 개체](#43924-타원-개체) - 페이지 43
          - [4.3.9.2.5. 다각형 개체](#43925-다각형-개체) - 페이지 45
          - [4.3.9.2.6. 호 개체](#43926-호-개체) - 페이지 45
          - [4.3.9.2.7. 곡선 개체](#43927-곡선-개체) - 페이지 45
        - [4.3.9.3. 흔글 수식 개체](#4393-흔글-수식-개체) - 페이지 46
        - [4.3.9.4. 그림 개체(HWPTAG_SHAPE_COMPONENT_PICTURE)](#4394-그림-개체hwptag_shape_component_picture) - 페이지 46
        - [4.3.9.5. OLE 개체(HWPTAG_SHAPE_COMPONENT_OLE)](#4395-ole-개체hwptag_shape_component_ole) - 페이지 49
        - [4.3.9.6. 차트 개체](#4396-차트-개체) - 페이지 50
        - [4.3.9.7. 묶음 개체(HWPTAG_SHAPE_COMPONENT_CONTAINER)](#4397-묶음-개체hwptag_shape_component_container) - 페이지 51
        - [4.3.9.8. 동영상 개체(HWPTAG_VIDEO_TDATA)](#4398-동영상-개체hwptag_video_tdata) - 페이지 51
      - [4.3.10. 개체 이외의 컨트롤](#4310-개체-이외의-컨트롤) - 페이지 52
        - [4.3.10.1. 구역 정의](#43101-구역-정의) - 페이지 53
          - [4.3.10.1.1. 용지 설정](#431011-용지-설정) - 페이지 54
          - [4.3.10.1.2. 각주/미주 모양](#431012-각주미주-모양) - 페이지 55
          - [4.3.10.1.3. 쪽 테두리/배경](#431013-쪽-테두리배경) - 페이지 56
        - [4.3.10.2. 단 정의](#43102-단-정의) - 페이지 56
        - [4.3.10.3. 머리말/꼬리말](#43103-머리말꼬리말) - 페이지 57
        - [4.3.10.4. 각주/미주](#43104-각주미주) - 페이지 57
        - [4.3.10.5. 자동 번호](#43105-자동-번호) - 페이지 58
        - [4.3.10.6. 새 번호 지정](#43106-새-번호-지정) - 페이지 58
        - [4.3.10.7. 감추기](#43107-감추기) - 페이지 58
        - [4.3.10.8. 홀/짝수 조정](#43108-홀짝수-조정) - 페이지 59
        - [4.3.10.9. 쪽 번호 위치](#43109-쪽-번호-위치) - 페이지 59
        - [4.3.10.10. 찾아보기 표식](#431010-찾아보기-표식) - 페이지 59
        - [4.3.10.11. 책갈피](#431011-책갈피) - 페이지 60
        - [4.3.10.12. 글자 겹침](#431012-글자-겹침) - 페이지 60
        - [4.3.10.13. 덧말](#431013-덧말) - 페이지 60
        - [4.3.10.14. 숨은 설명](#431014-숨은-설명) - 페이지 60
        - [4.3.10.15. 필드 시작](#431015-필드-시작) - 페이지 61
    - [4.4. 문서 이력 관리](#44-문서-이력-관리) - 페이지 62
      - [4.4.1. 문서 이력 관리란](#441-문서-이력-관리란) - 페이지 62
      - [4.4.2. 문서 이력 관리 레코드 정보](#442-문서-이력-관리-레코드-정보) - 페이지 62
        - [4.4.2.1. 히스토리 아이템 정보 시작](#4421-히스토리-아이템-정보-시작) - 페이지 62
        - [4.4.2.2. 히스토리 아이템 정보 끝](#4422-히스토리-아이템-정보-끝) - 페이지 62
        - [4.4.2.3. 히스토리 아이템 버전](#4423-히스토리-아이템-버전) - 페이지 63
        - [4.4.2.4. 히스토리 날짜](#4424-히스토리-날짜) - 페이지 63
        - [4.4.2.5. 히스토리 작성자](#4425-히스토리-작성자) - 페이지 63
        - [4.4.2.6. 히스토리 설명](#4426-히스토리-설명) - 페이지 63
        - [4.4.2.7. 비교 정보](#4427-비교-정보) - 페이지 63
        - [4.4.2.8. 가장 마지막 최근 문서](#4428-가장-마지막-최근-문서) - 페이지 63
      - [변경 사항 이력](#변경-사항-이력) - 페이지 65
...
| INT8 | 1 | 밝기 |
| INT8 | 1 | 명암 |
| BYTE | 1 | 그림 효과<br>- 0: REAL_PIC<br>- 1: GRAY_SCALE<br>- 2: BLACK_WHITE<br>- 4: PATTERN8x8 |
...
- **확장 레벨(8~10) 결정 방법 / Extended Level (8~10) Determination**:
  - `paragraph_level`이 6 (bit 25-27 = 6)이고 `line_spacing`이 7 이상인 경우, 확장 레벨(8~10)로 판단합니다.
  - 확장 레벨의 실제 레벨을 결정할 때는 `para_style_id`를 사용하여 스타일 이름에서 레벨을 추출합니다.
    - 예: 스타일 이름이 "개요 8"이면 레벨 8, "개요 9"이면 레벨 9, "개요 10"이면 레벨 10
  - 스타일 이름에서 레벨을 추출할 수 없는 경우, `line_spacing + 1`을 레벨로 사용합니다.
  - `extended_levels` 배열 범위를 벗어나는 경우 (예: 레벨 10인데 `extended_levels`에 2개만 있는 경우), 넘버링 없이 텍스트만 표시합니다.
  - 이 동작은 스펙 문서에 명시되어 있지 않지만, 실제 HWP 파일의 동작을 기반으로 합니다.
...
| INT32 | 4 | 왼쪽 여백 |
| INT32 | 4 | 오른쪽 여백 |
| INT32 | 4 | 들여쓰기/내어쓰기 (양수=들여쓰기, 음수=내어쓰기, 원본 스펙에는 별도 필드로 기재되어 있으나 실제 데이터는 단일 필드) |
| INT32 | 4 | 문단 위 간격 |
| INT32 | 4 | 문단 아래 간격 |
...
| 범위 | 설명 |
|---|---|
| bit 0-1 | 줄 간격 종류(한글 2007 이하 버전)<br>- 0: 글자에 따라(%)<br>- 1: 고정값<br>- 2: 여백만 지정 |
| bit 2-4 | 정렬 방법<br>- 0: 양쪽 정렬<br>- 1: 왼쪽<br>- 2: 오른쪽<br>- 3: 가운데<br>- 4: 배분<br>- 5: 나눔 |
| bit 5-6 | 줄 나눔 기준 영문 단위<br>- 0: 단어<br>- 1: 하이픈<br>- 2: 글자 |
| bit 7 | 줄 나눔 기준 한글 단위<br>- 0: 단어 단위<br>- 1: 글자 |
...
| bit 20-21 | 세로 정렬<br>- 0: 글꼴 기준선<br>- 1: 위<br>- 2: 가운데<br>- 3: 아래 |
| bit 22 | 글꼴에 맞는 줄 높이 |
| bit 23-24 | 문단 머리 모양 종류<br>- 0: 없음<br>- 1: 개요<br>- 2: 번호<br>- 3: 글머리표 |
| bit 25-27 | 문단 수준(1~7) |
| bit 28 | 문단 테두리 연결 |
| bit 29 | 문단 여백 무시 |
| bit 30 | 문단 꼬리 모양 |
...
| 자료형 | 길이(바이트) | 설명 |
|---|---|---|
| UINT32 | 4 | text(=chars)<br>`if (nchars & 0x80000000) { nchars &= 0x7fffffff; }` |
| UINT32 | 4 | control mask<br>`(UINT32)(1<<ctrich) 조합`<br>`ctrich는 HwpCtrlAPI, Hwp 2.1 CtrlCh 참고` |
...
| BYTE stream | n3 | 셀 리스트(표 79 참조)<br>셀 sizex셀 개수 |
| 전체 길이 | 가변 | `n + n2 + n3` |
...
| 자료형 | 길이(바이트) | 설명 |
|---|---|---|
| WORD | 2 | scale matrix와 rotation matrix쌍의 개수(cnt)<br>초기엔 1, group할 때마다 하나씩 증가하고, ungroup할 때마다 하나씩 감소한다. |
...
| INT32 | 4 | 끝점 X 좌표 |
| INT32 | 4 | 끝점 Y 좌표 |
| UINT16 | 2 | 속성<br>처음 생성 시 수직 또는 수평선일 때, 선의 방향이 언제나 오른쪽(위쪽)으로 잡힘으로 인한 현상 때문에, 방향을 바로 잡아주기 위한 플래그 |
| 전체 길이 | 18 | |
...
| 자료형 | 길이(바이트) | 설명 |
|---|---|---|
| BYTE | 1 | 사각형 모서리 곡률(%)<br>직각은 0, 둥근 모양은 20, 반원은 50, 그 외는 적당한 값을 % 단위로 사용한다. |
...
| 범위 | 설명 |
|---|---|
| bit 0-7 | drawing aspect<br>- DVASPECT_CONTENT = 1<br>- DVASPECT_THUMBNAIL = 2<br>- DVASPECT_ICON = 4<br>- DVASPECT_DOCPRINT = 8<br>자세한 설명은 MSDN의 MFC COleClientItem::m_nDrawAspect 참고 |
...
| 자료형 | 길이(바이트) | 설명 |
|---|---|---|
| UINT | 2 | 속성<br>- 감출 대상<br>  - 0x00000001: 머리말<br>  - 0x00000002: 꼬리말<br>  - 0x00000004: 바탕쪽<br>  - 0x00000008: 테두리<br>  - 0x00000010: 배경<br>  - 0x00000020: 쪽 번호 위치 |
| 전체 길이 | 2 | |
...
| 자료형 | 길이(바이트) | 설명 |
|---|---|---|
| UINT32 | 4 | 속성<br>- bit 0-1: 홀/짝수 구분<br>  - 0: 양쪽<br>  - 1: 짝수 쪽<br>  - 2: 홀수 쪽 |
| 전체 길이 | 4 | |
...
책갈피 컨트롤은 '책갈피 이름'만 정보로 가지고 있으며, 임의의 컨트롤 데이터 `HWPTAG_CTRL_DATA`로 기록된다.
...
| WORD | 2 | sub Text 길이 |
| WCHAR | 2 | sub Text |
| UINT | 4 | 댓말의 위치<br>- 0: 위<br>- 1: 아래<br>- 2: 가운데 |
| UINT | 4 | Fsizeratio |
| UINT | 4 | Option |
| UINT | 4 | Style number |
| UINT | 4 | 정렬 기준<br>- 0: 양쪽 정렬<br>- 1: 왼쪽 정렬<br>- 2: 오른쪽 정렬<br>- 3: 가운데 정렬<br>- 4: 배분 정렬<br>- 5: 나눔 정렬(공백에만 배분) |
| 전체 길이 | 18 | |
...
문서 이력 정보의 각 항목은 "히스토리" 또는 "히스토리 아이템"이라고 불리며, 한글 Compound 구조 내부의 "DocHistory"라는 이름의 스토리지 안에 `VersionLog%d`(`%d`는 버전을 나타냄)라는 이름의 스트림으로 저장된다. 이 항목들은 압축되고 암호화된다.
