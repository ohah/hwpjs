...
`hwp5.document_validation.inspectDecoded`는 압축 해제된 DocInfo·구역에 기존 검사기를 연결하고 구역 수/인덱스·전역 바이트/레코드 한도를 검증합니다. 보고서는 구역 순서와 개별 보류 항목을 유지합니다. CFB 스트림 검색·압축 해제·BinData 실물 연결까지 수행하는 파일 단위 진입점은 아직 아닙니다.
