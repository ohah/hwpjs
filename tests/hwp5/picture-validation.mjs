...
export function pictureOwnerActual(call,v,b,mode=0){
  const stage=mode%6,nodes=[],stack=[],stats=[0,0,0,0,0,0,0];
  ...
  for(const [i,n] of nodes.entries()){
    ...
    stats[0]++;stats[1]+=Number(p[70]>3);stats[2]++;
  ...
  assert.deepEqual(pictureOwnerRun(call,v,b,mode),Buffer.concat(stats.map(w)));return stats;
}
export function pictureOwnerEdges(call){
  ...
  for(let mode=0;mode<12;mode++){
    ...
    assert.deepEqual(check(good),[1,1,1,Number(stage>=3),Number(stage>=4),Number(stage===5),95-[73,74,78,82,90,91][stage]]);
