...
export function decodeEmfReport(bytes) {
  if (!(bytes instanceof Uint8Array) || bytes.length < 40 || bytes.length % 4) throw new Error('InvalidEmfReport');
  const b=Buffer.from(bytes),records=b.readUInt32LE(0);
  if (b.length !== (10+records)*4) throw new Error('InvalidEmfReport');
  return {
    ...
    eofPaletteEntries:b.readUInt32LE(36),recordTypes:Array.from({length:records},(_,i)=>b.readUInt32LE(40+i*4)),
  };
}
