...
export function attainedInverseEdges(call,bits=128){
  assert.ok(bits===128||bits===512);
  const mode=bits===128?202:231,width=bits/4;
  const input=(kind,raw,n,d,precision=512)=>preimageInput(kind,raw,n,d,precision,bits);
  let selected=0,missing=0,rejected=0,undecided=0;
  function check(kind,raw,n,d,x,precision=512){const out=call(mode,input(kind,raw,n,d,precision));if(x===null){assert.deepEqual(out,Buffer.alloc(4));missing++;}else{assert.equal(out.length,32+2*width);assert.equal(out.readUInt32LE(),2);endpoint(out.subarray(4),x,true,width);selected++;}}
  function reject(kind,raw,n,d,error,precision=512){assert.throws(()=>call(mode,input(kind,raw,n,d,precision)),new RegExp(error));rejected++;}
  ...
  const max=(1n<<BigInt(bits))-1n,n=65537n*65537n<<BigInt(bits-64);
  assert.deepEqual(call(mode,input(3,[131072,65537,0,0,1],n,max,128)),Buffer.from([1,0,0,0]));undecided++;
  check(3,[65536,65536,0,65537,65537],max/2n,max,[65536n*(max/2n),65537n*max]);
  if(bits===512){
    for(const [a,b] of [[131072,-32768],[-131072,98304],[65537,0]])for(const y of [0n,1n,max/2n,max-1n,max]){
      const interval=linearPreimageReference({start:[0n,1n],end:[1n,1n],flags:3,a,b},[y,max]);
      check(3,[65536,a,b,0,0],y,max,interval?choiceReference(interval):null,1024);
    }
    check(0,[131072],1n<<509n,1n<<511n,[1n,2n],1024);
    check(3,[-65536,65536,65536,0,0],1n<<510n,1n<<511n,[1n,1n],1024);
  }
  ...
  for(const [b,l,e] of [[good,good.length-1,/LimitExceeded/],[Buffer.concat([good,Buffer.alloc(1)]),undefined,/InvalidIcc/],[input(0,[65536],1n,3n,64),undefined,/InvalidIccComparisonPrecision/]]){assert.throws(()=>call(mode,b,l),e);rejected++;}
