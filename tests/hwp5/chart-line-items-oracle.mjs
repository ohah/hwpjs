...
export function lineItemsOracle(b,count=2){
 ...
 const wire=lineItemsWire(rows,word,offset,types.size,objects.size);
 const input=(bytes=b,maxObjects=objects.size,n=count)=>Buffer.concat([ints([maxObjects,n]),bytes]);
 return {wire,input,rows,axis,word,start:axis.end,end:offset,objects:objects.size};
}
export function lineItemsWire(rows,word,end,typeCount,objectCount){
 return Buffer.concat([ints([word,rows.length,end,typeCount,objectCount]),...rows.map(r=>Buffer.concat([ints([r.objectId,r.end]),Buffer.from(r.raw52,'hex')]))]);
}
