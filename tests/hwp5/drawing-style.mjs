...
export function drawingStyleActual(call,b,mode=1) {
  const width=mode?13:11,flags=b.readUInt32LE(width),known=(flags&~7)===0;
  const fields=[known?1:0,flags,b.readUInt32LE(0),mode?b.readInt32LE(4):b.readInt16LE(4),b.readUInt32LE(mode?8:6),b[width-1]];
  let at=width+4;
  if(known){
    if(flags&1)at+=12;
...
export function drawingStyleEdges(call){
  ...
  for(const mode of [0,1])for(let flags=0;flags<8;flags++){
    const border=Buffer.alloc(mode?13:11),parts=[border,w(flags)];
  ...
  for(const mode of [0,1]){
    const border=Buffer.alloc(mode?13:11);
