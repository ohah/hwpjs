...
pub fn run(a: std.mem.Allocator, bytes: []const u8, limit: usize) ![]u8 {
    ...
    inline for (.{ "VtChartFootnote\x00", "VtChartText\x00" }) |name| {
        const ref = try grid.prelude.types.readObserved16(&reader);
    ...
    const block = try core.hwp5.chart_text_block.readObservedV2(&reader, &grid.prelude.types, .{ .max_string_bytes = max_string, .max_total_string_bytes = max_total });
