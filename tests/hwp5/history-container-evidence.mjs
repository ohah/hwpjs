...
export function historyContainerEvidence(base,model,o={}) {
  ...
  if(o.last_document&&last){const b=o.mode===1?Buffer.from(last.content):inflateRawSync(last.content);lastDocument=lastDocumentEvidence(b);decoded+=b.length;records+=lastDocument.records;}
  const changed=consumeContainerStreams(base,decoded,entries.length+Number(lastDocument!==null));
  const header=[1,Number(root>=0),declared,Number(!!last),last?.content.length??0,decoded,records,entries.length];
  return {wire:Buffer.concat([changed,...header.map(w),...entries.flat().map(w)]),entries,decoded,records,lastDocument};
}
