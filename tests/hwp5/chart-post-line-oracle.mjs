...
export function postLineOracle(b){
 ...
 const wire=Buffer.concat([...[r.end,r.types.size,r.objects.size,r.array.id,r.array.first,r.array.second,r.end].map(n=>integer(n)),Buffer.from(r.raw194,'hex')]);
 const input=(bytes=b,maxObjects=r.objects.size)=>Buffer.concat([integer(maxObjects),bytes]);
 return {wire,input,r,start:prior.end,end:r.end,objects:r.objects.size,priorObjects:last.objects};
}
