...
export function xmlTagEvidence(source,e,limit=67108864){
  ...
  return {wire:Buffer.concat(out),kind,name:tagName,units:at,bytes:raw.length,characters,attributes:attrs.length,attributeValues:attrs,references,unresolved,maxNameBytes:Math.max(nameBytes.length,...attrs.map(a=>xmlEncode(a.name,e).length))};
}
