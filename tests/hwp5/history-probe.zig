...
pub fn run(a: std.mem.Allocator, bytes: []const u8, limit: usize) ![]u8 {
    ...
    if (mode > 2) return error.InvalidHistoryProbeMode;
    ...
    const layout: core.hwp5.history.value.StartLayout = if (mode == 2) .observed_option_first else .spec_flag_first;
    var out: std.ArrayList(u8) = .empty;
    errdefer out.deinit(a);
    if (mode != 0) {
        const item = try core.hwp5.history.Item.parse(raw, .{ .start_layout = layout, .framing = options });
