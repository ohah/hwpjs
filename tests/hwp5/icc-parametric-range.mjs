...
export function parametricRangeEdges(call){let ranges=0,rejected=0;
 function check(kind,raw,precision=512){const input=powerRangeInput(kind,raw,precision);let active;try{active=domain(kind,raw);}catch(e){assert.throws(()=>call(207,input),new RegExp(e.message));rejected++;return;}
  ...
  else {const expected=linearRange({start:[0n,1n],end:active?active.start:[1n,1n],flags:active?1:3,a:kind>=3?raw[3]:0,b:kind===2?raw[3]:kind===4?raw[6]:0});assert.equal(lower.readUInt32LE(),1);assert.equal(lower.readUInt32LE(4),expected.flags);const start=[readWide(lower,8),readWide(lower,40)],end=[readWide(lower,72),readWide(lower,104)];assert.ok(start[1]>0n&&end[1]>0n);assert.equal(cmp(start,expected.start),0);assert.equal(cmp(end,expected.end),0);}
