...
export function xmlDocumentEvidence(original,byteLength){
  ...
  while(at<text.length){
    ...
    if(text.startsWith('<!--',at)){
      ...
      const end=text.indexOf('?>',at+2);assert.ok(end>=0);const body=text.slice(at+2,end),name=xmlNamePattern.exec(body)?.[0];assert.ok(name&&name.toLowerCase()!=='xml');assert.ok(body.length===name.length||/[ \t\n]/.test(body[name.length]));pi++;at=end+2;
    }else if(text[at]==='<'){
      assert.ok(!text.startsWith('<!',at));const tag=xmlTagEvidence(text.slice(at),0);assert.equal(tag.unresolved,0);attributes+=tag.attributes;references+=tag.references;
