...
export function lineOwnerActual(call,v,b){
  ...
  for(const [i,n] of nodes.entries()){
    if(n.tag===78){
      ...
      if(owner===0x24636f6c)stats[1]++;
    }
    if(identity(n)!==0x246c696e)continue;
    const children=nodes.filter(c=>c.parent===i&&c.tag===78);assert.equal(children.length,1);
    const c=children[0];assert.ok(c.end-c.start>=18);stats[0]++;
    stats[2]+=Number(b.readUInt16LE(c.start+16)>1);stats[3]+=c.end-c.start-18;
  }
...
export function lineOwnerEdges(call){
  ...
  check(good);check(Buffer.concat([owner(0x24636f6c),line(1,Buffer.alloc(0))]));
  ...
  const p=Buffer.alloc(21);p.writeUInt16LE(65535,16);assert.deepEqual(check(Buffer.concat([owner(),line(1,p)])),[1,0,1,3]);
