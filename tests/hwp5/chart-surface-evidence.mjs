...
export function observeSurfacePrefix(bytes,offset,priorTypes){
 ...
 const types=new Map(priorTypes),declarations=[];let p=offset;
 ...
 const type=name=>{const id=long();if(!types.has(id)){const n=word(),nameOffset=p,raw=take(n).toString('latin1'),versionOffset=p,version=word();types.set(id,{name:raw,version});declarations.push({id,nameOffset,versionOffset,name:raw,version});}const d=types.get(id);if(d.name!==name+'\0'||d.version!==1)throw Error('UnsupportedSurfaceObservationType');return id;};
 ...
 const arrayTypeId=type('VtArray'),first=word();type('VtCollection');const second=word();type('VtObject');
 if(first!==0||second!==0)throw Error('UnsupportedSurfaceObservationArray');
 return {start:offset,raw30,surfaceStart,objectId,typeId,bodyStart,raw46,array:{start:arrayStart,id:arrayId,typeId:arrayTypeId,first,second,end:p},end:p,declarations};
}
