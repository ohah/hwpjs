...
export function pictureSelectedDocument(call,cfb){
  ...
  for(let mode=0;mode<6;mode++){
    ...
    const report=run(b),container=full(b),expected=pictureOwnerActual(call,v,b,mode);
    assert.deepEqual(expected,[2,0,2,mode>=3?2:0,mode>=4?2:0,mode===5?2:0,2*(91-[73,74,78,82,90,91][mode])]);
