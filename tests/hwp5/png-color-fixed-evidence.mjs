...
export function colorFixedEvidence({chunks}){
  const fields=Array(11).fill(0);let count=0,bytes=0;
  ...
  return {fields,count,bytes};
}
export function colorFixedWire(t){const out=Buffer.alloc(56);[...t.colorFixed.fields,Number(t.colorSemanticsDeferred),t.deferredChunks,t.deferredBytes].forEach((v,i)=>out.writeUInt32LE(v,i*4));return out;}
