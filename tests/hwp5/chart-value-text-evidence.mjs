...
export function observeValueText(bytes,offset,priorTypes,priorStrings,priorNumbers=new Map()){
 const prefix=observeValuePrefix(bytes,offset,priorTypes,priorStrings,priorNumbers),b=Buffer.from(bytes);
 ...
 for(const s of [prefix.reference,prefix.format?.code,prefix.label])if(s&&s.kind!=='number')strings.set(s.id,{hex:s.hex,trailer:s.trailer});
