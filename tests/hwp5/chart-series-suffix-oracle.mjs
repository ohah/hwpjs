...
export function seriesSuffixOracle(b){
 ...
 for(const state of r.formats){const f=state.format,s=f.code,raw=Buffer.from(s?.hex??'','hex');parts.push(...[f.headerWord,f.rawWord,Number(s!==null),s?.id??0xffffffff,raw.length,s?.trailer??0,Number(s?.introduced??false),f.codeStart,f.codeEnd,f.end,state.objects.size,stored(state.strings)].map(n=>integer(n)),raw);}
