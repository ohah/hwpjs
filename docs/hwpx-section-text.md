...
[한컴의 HWPX 구조 설명](https://tech.hancom.com/hwpxformat/)은 section의 `hp:p` 아래 `hp:run`과 `hp:t`가 본문 텍스트를 담는다고 설명합니다. `Document.inspectSectionText`는 기존 [header·spine 구조](hwpx-document-structure.md)가 고른 평문 section만 spine 순서로 다시 읽고, 2011 paragraph namespace의 `p`·`run`·`t`를 구분합니다. 한컴의 후속 본문 설명에는 2021 namespace의 스키마 예도 있지만, 이를 이 API의 2021 지원 증거로 사용하지 않습니다. `hp:t`의 XML 문자 참조·CDATA·줄바꿈 정규화가 끝난 UTF-8 내용과 내부 요소 시작·끝·빈 태그를 원래 순서대로 콜백에 전달합니다. 콜백의 `Tag`·namespace scope·텍스트 바이트는 호출 중에만 유효하므로 보관하려면 호출자가 복사해야 합니다. 파싱 또는 콜백 오류 전까지의 이벤트는 이미 전달됐을 수 있으므로 호출자는 실패한 스트림의 부분 상태를 버려야 합니다. section의 manifest item 인덱스와 section·문단·run·text의 관측 순번을 이벤트에 포함합니다. 실제 편집 가능한 문서 모델은 아직 만들지 않습니다.
...
2026-09-27 현재 기본 `inspectSectionText`의 전 분기 관측과 콜백 수명·한도를 코드에서 다시 확인하고, 한컴의 후속 2021 namespace 스키마 예를 현재 2011 지원 범위와 구분했습니다. `HWPX section text` 집중 테스트는 Debug·ReleaseSafe·ReleaseFast 각 15개가 통과했고, 직접 run 없는 실파일 회귀도 ReleaseFast에서 통과했습니다. 독립 ZIP/XML 조사기의 일반·`-O` 전체 결과와 제품의 476개 수용·544개 section·215,146개 문단·267,347개 run·230,677개 `t`·7,975,957바이트가 일치했으며, 직접 run 없는 문단은 여전히 1개입니다. 독립 조사기의 Python `assert` 자체 반례를 최적화 모드에서도 작동하게 바꾸고 늦은 ZIP 오류의 파일 단위 부분 집계도 되돌리도록 수정했습니다. 이 결과는 XML 이벤트 관측이며 렌더링·편집·저장과 2021 namespace 지원을 증명하지 않습니다.
