...
section 트리의 모든 `hp:p`에 적용되는 [문단 직접 자식 구조](hwpx-paragraph-children.md)는 셀 목록 안의 문단도 포함합니다. 이는 `run`·`linesegarray`의 직접성 진단이며 셀 본문의 의미 조립은 아닙니다.
...
2026-09-27 현재 내용 재검증: 고정 버전 모델과 직접 자식 선택 코드, 독립 조사기 자체 반례 및 476개 실파일 집계를 다시 대조했습니다. 106,998개 셀·직접 목록에서 누락 셀과 중복 목록 셀은 각각 0개이고, 직접 문단은 128,104개입니다. `id` 부재 59개·빈 값 106,939개, `metatag` 존재 100개, 나머지 9개 속성 전부 존재하며 목록의 다른 직접 요소·미지 열거값은 0개입니다. 집중 테스트는 Debug·ReleaseSafe·ReleaseFast에서 각각 5개, 공통 ParaListType 트리 테스트는 Debug에서 3개 통과했습니다. 기존 known-inspections 8개 shard는 제품 코드가 바뀌지 않아 이전 실행 결과를 재사용했으며 이 날짜에 다시 실행한 것으로 세지 않습니다.
