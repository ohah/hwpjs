...
현재 로컬 HWP corpus에는 EMF+ signature가 없어 실제 한컴 DrawBeziers 표본과 렌더링 결과는 관측하지 못했습니다. 구현 범위는 wire 구조, borrowed point iteration, [공용 상대좌표 누적](emf-plus-point-resolution.md), 연결 cubic segment 조립, [일반 world/page/device 변환과 segment별 평가·flattening](emf-plus-bezier-device-segments.md), [record 전체 connected polyline](emf-plus-bezier-device-polyline.md), Object Table 참조와 stream/framing 연결입니다. clip, stroke·래스터화와 저장은 미구현입니다.
