...
마스터페이지 스냅샷의 전수 조사와 함께 내용·이벤트 순서 해시 및 보고서 계수 검사는 `src/hwpx_text_snapshot_corpus_common.zig`로, 독립 Python XML 이벤트 규칙은 `tools/hwpx_text_snapshot_order.py`로 옮겼습니다. section의 spine 파트 선택과 마스터페이지의 manifest 파트 선택은 각각의 조사기에 남겨 두었습니다. 공통화 후 기존 section 원문·기본 선택·차트 선택 세 모드의 476개 정상 파일별 대조를 모두 다시 실행했고, 기존 합계와 일치했습니다. Python 검증기 음성 대조도 일반 모드와 `-O`에서 통과했습니다.
...
2026-09-27 현재 내용 재검증: 현재 스냅샷의 태그·정규화 내용 소유, 이벤트/복사 바이트 한도, 실패 해제 경계를 대조하고 집중 테스트를 Debug·ReleaseSafe·ReleaseFast 각 5개 실행했습니다. 단일 추적 실파일의 607바이트·고정 SHA-256과 ReleaseFast 보고서도 다시 일치했습니다. 검증기 음성 반례는 일반 Python과 `-O`에서 통과했고, 로컬 484개 후보의 원문·기본 선택·차트 선택 세 모드를 각각 독립 ZIP/XML의 파일별 내용·순서 해시 및 여섯 계수와 다시 대조했습니다. 세 모드 모두 정상 476개·ZIP 거부 6개·암호화 2개였으며, 원문은 section/문단/run/`hp:t`/빈 `hp:t`/본문 바이트 544/215,146/267,347/230,677/14,607/7,975,957, 두 선택 모드는 각각 544/215,144/267,345/230,675/14,607/7,975,784였습니다. 같은 합계가 각 선택 모드의 콘텐츠나 화면 표시 동치를 뜻하지는 않습니다.
