...
[world/page transform과 보수적 clip snapshot](emf-plus-graphics-state.md) 생성과 EndContainer 복원뿐 아니라 [DestRect/SrcRect/PageUnit 컨테이너 transform](emf-plus-container-transform.md)도 tracked stream에 연결했습니다. World/Display는 SHOULD NOT 호환 입력으로 보존하되 transform unknown으로 보고합니다. quality 등 다른 graphics 속성 및 렌더링은 아직 구현하지 않았습니다. [BeginContainerNoParams](emf-plus-begin-container-no-params-record.md)와 [EndContainer](emf-plus-end-container-record.md)는 같은 stack에 연결되어 wire 수명주기를 검증합니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
