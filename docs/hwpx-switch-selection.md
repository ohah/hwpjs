...
2026-09-27 현재 내용 재검증: 한컴 고정 리비전 handler의 첫 일치 분기·공백 전용 요구값 처리와 현재 공유 선택 정책·두 참조 API의 위임·수명 경계를 대조했습니다. 독립 XML 조사에서는 허용 476개 파일의 직접 `switch` 93개가 모두 case/chart와 default/OLE 한 쌍이었으며, 선택 결과를 전체 문서 의미로 확대하지 않습니다. 선택 참조 집중 테스트는 Debug·ReleaseSafe·ReleaseFast 각 9개, 원문 이진·차트 회귀는 Debug 7/16개가 통과했습니다. 이전 known-inspections 8개 shard는 제품 코드가 바뀌지 않아 결과를 재사용했고 이 날짜에 다시 실행한 것으로 세지 않습니다.
