...
직전 파트의 `basic_point_drawing.zig`와 `basic_shapes.zig`도 같은 SSOT를 사용한다. count-derived array, offset/size section, 가변 객체를 포함한 palette·poly·font·bitmap brush·extended pen·color-space creation은 의미 payload 끝과 보존할 extra를 먼저 분리해야 하므로 이 파트의 완료 범위가 아니다.
