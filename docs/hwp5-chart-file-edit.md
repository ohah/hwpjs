...
실제 고정 Contents의 위 지원 inventory 60개, [축 Double 선택 값](hwp5-chart-number-edit.md) 2개, [Grid Double 셀](hwp5-chart-grid-number-edit.md) 12개와 [Grid String 셀](hwp5-chart-grid-string-edit.md) 7개, 총 81개를 한 `applyEdits` 호출에 넣고 바깥 HWP, 압축 BinData, 내부 OLE와 Contents를 다시 열어 전 위치를 확인합니다. 이 수치는 현재 관측·지원된 문자열·형식·숫자 편집 대상의 완전 batch이며, 차트의 opaque raw 필드나 아직 의미를 부여하지 않은 다른 객체를 편집한다는 뜻은 아닙니다.
