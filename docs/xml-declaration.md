...
문서 entity의 BOM과 선택적 XMLDecl을 처리하는 시작 계층입니다. `prolog.open`이라는 이름이 전체 Prolog 문법(Misc/DTD 포함)의 완료를 뜻하지 않습니다. 루트 요소·태그·일반 PI는 [문서 구조 검증](xml-document.md), 선택적 namespace 검사는 [namespace 계층](xml-namespaces.md)과 연결됩니다. DTD·external TextDecl·스키마와 이력 복원은 아직 미구현입니다. 빈 입력도 시작 계층에서는 반환될 수 있으며 문서 유효성은 문서 계층이 판단합니다.
