...
이 모듈은 밑 변수의 해 생성만 담당합니다. 1024비트 근 비교·수치 평가, affine x 역변환, 활성 분기 포함 검사, 출력 클리핑, 최근접 출력 선택, 넓은 parametric/TRC 모델 조립은 별도 구현입니다. 기존 u256 근 비교기에 좁혀 전달하지 않습니다. 전체 ICC 렌더링이나 HWP/HWPX 문서 검증 완료를 뜻하지 않습니다.
