...
이 계층은 명시적으로 닫힌 figure의 topology만 구현합니다. FillPath가 열린 figure도 채울 때 적용하는 암묵적 직선은 별도 [Path fill boundary 계층](emf-plus-path-fill-segments.md)이 소유합니다. DashMode, marker, transform, clipping, fill rule, Pen cap/join, 곡선 평가·flattening, rasterization과 저장도 후속 범위입니다. 로컬 지원 HWP corpus에는 EMF+ signature 표본이 0개라 실제 한컴 렌더링 동등성은 주장하지 않습니다.
