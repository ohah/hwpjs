...
이 계층은 Path command의 device 좌표와 metadata까지만 제공합니다. figure별 소유 aggregate·polyline, 명시적/암묵적 closure, DashMode 적용, marker 소비, Pen cap/join, fill rule, clipping, rasterization과 저장은 후속 책임입니다. Object Table이 Path payload를 장기 소유하지 않으므로 DrawPath·FillPath record replay가 이 API를 자동 호출하지 않습니다. 로컬 지원 HWP corpus에 EMF+ signature 표본이 없어 실제 한컴 렌더링 동등성을 주장하지 않습니다.
