...
2026-09-27 재검증: `HWPX note` 필터의 Debug·ReleaseSafe·ReleaseFast가 각각 27/27 통과했습니다. 독립 위치 오라클의 일반·`-O` 자체검사는 구조 변이 8개와 UTF-16 양 바이트 순서를 검출했고, 전체 실행은 허용 476개·ZIP 거부 6개·암호화 2개를 파일별로 일치시켰습니다. 직접 `ctrl`·문단·run 조상은 각 1,219건, `subList` 조상은 52건, 텍스트·바깥 주석 조상은 0건으로 기존 기록과 같습니다. 같은 코드·corpus의 known-inspections ReleaseFast 8개 shard도 각각 통과했습니다. 이 검사는 원본 XML 사이트에 한정됩니다.
