...
후속 [이미지 ID 대상 연결](hwpx-fill-brush-image-links.md)은 이 원값 보고서의 이미지 노드를 OPF 항목에 별도로 연결합니다. 원값 보고서 자체가 OPF를 해결하지 않는 계약은 유지합니다. payload 검증, brush의 실제 합성 우선순위·색 관리·투명도, 편집·저장·무손실 왕복은 남아 있습니다. 이 원값 검사를 전체 HWPX 문서 유효성 판정으로 사용하지 않습니다.
