# 차트 Font String 참조 분리

## 계약

`chart/contents_string_fork.zig`의 `forkFontName`은 기존 String object ID를 재참조하는 Font 이름 한 곳을 새 inline String 정의로 교체합니다. 대상은 `name_introduced == false`이고 [보존된 참조 span](hwp5-chart-string-reference-spans.md)이 정확히 4바이트여야 합니다. span의 u32 ID, Font 모델의 ID, 객체 사전의 String entry가 모두 일치해야 합니다.
...
SHA-256 고정 9,876바이트 Contents의 첫 주축 Font 이름 alias를 새 ID `0xfffffffe`, 다른 길이의 문자열과 trailer로 분리합니다. 출력 길이와 extent를 확인하고 전체 Contents를 다시 파싱하여 다음을 검증합니다.
...
null ID, 기존 ID 충돌, inline 정의 대상, 65,536바이트 입력, 잘못된 span·원본 ID·객체 사전 entry, 필수 타입 version 부재는 정확한 오류로 거부합니다.
...
현재 adapter는 span을 직접 보존하는 Font 이름만 지원합니다. 자동 object ID 탐색, 새 type ID·선언 생성, Font 밖 참조의 분리, 문자열 인코딩 변환, 여러 편집의 일괄 트랜잭션, CFB 스트림 저장은 제공하지 않습니다.
