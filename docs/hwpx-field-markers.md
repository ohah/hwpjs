...
2026-09-26 기준 Debug 전체 `zig build test --summary all`은 최종 코드 상태에서 종료 코드 0, 2,609/2,609 테스트 및 5/5 빌드 단계를 통과했습니다. 집중 테스트는 Debug·ReleaseSafe·ReleaseFast 각각 9/9, ReleaseSafe 제품 빌드와 `zig fmt --check build.zig src`, 독립 Python oracle의 일반/`-O` 변이 self-test, 전체 476개 허용 파일별 해시 대조, 기존 known 문서 샤드 0~7도 통과했습니다. 전체 테스트 출력에는 `failed command:` 문자열이 한 줄 있으나 같은 실행의 최종 요약은 `test success`였습니다. 이 중간 출력의 출처는 확인되지 않았으므로 별도 실패로도 무시 가능한 진단으로도 단정하지 않습니다. 실파일 corpus와 known 연결은 기본 전체 테스트 밖의 선택 검사입니다.
