...
현재 구현은 page-to-device scale 상태와 수명주기를 재생합니다. [여덟 graphics property](emf-plus-property-state.md)도 별도 상태로 추적하지만 실제 geometry 좌표 적용, device origin과 rasterization은 구현 범위가 아닙니다. clip은 [별도 보수적 상태](emf-plus-clip-state.md)로 재생하지만 geometry clipping은 하지 않습니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력의 픽셀 동등성도 주장하지 않습니다.
