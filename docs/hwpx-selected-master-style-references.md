...
이 단계는 서식 **참조 ID** 연결이지 화면 표시·상속된 스타일 값·마스터페이지 적용 결과를 계산하지 않습니다. 원문 양쪽 분기 보고서의 진단을 선택된 문서의 적합성 진단으로 대체하지 않으며, 문서 모델·저장/편집은 아직 후속 범위입니다. 현재 서식 참조 스캐너는 `hp:t` 후손의 동명 `hp:run`도 run으로 셀 수 있지만 텍스트 스캐너는 이를 인라인으로 분류하므로, 실파일의 문단·run 수 일치를 임의의 중첩 XML 동치로 일반화하지 않습니다.
...
2026-09-27 현재 내용 재검증: 루트 직접 `subList` 범위·공유 선택 프레임·서식 ID 판독·파트/문단/run 예산 코드를 대조했습니다. 독립 XML 조사에서 허용 476개 문서의 마스터페이지 `switch`는 0개이므로 이 경로의 실파일 양성 선택 검증은 여전히 없습니다. 집중 테스트는 Debug·ReleaseSafe·ReleaseFast 각 6개, 원문 마스터페이지 서식 참조 Debug 6개가 통과했습니다. 이전 known-inspections 8개 shard는 제품 코드 불변을 확인해 결과만 재사용했고 이번에 재실행한 것으로 세지 않습니다. `hp:t` 안쪽 동명 `run`의 두 스캐너 범위 차이는 위 제한에 포함합니다.
