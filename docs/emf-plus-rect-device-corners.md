...
이 계층은 rectangle의 device-space 네 corner까지만 제공합니다. [rectangle record 연결](emf-plus-rect-record-device-corners.md)은 이 API를 재사용하고 Ellipse·Arc·Pie의 basis·exact conic·단일 parameter 평가는 각 전용 계층이 이어서 소유합니다. flattening, Pen/Brush 의미, clipping, rasterization과 저장은 후속 범위입니다. 로컬 지원 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성을 주장하지 않습니다.
