...
HWPX ZIP 진입 경계의 전용 테스트는 `zig test src/root.zig --test-filter HWPX`로 실행합니다. 실제 파일 두 개의 모든 ZIP 엔트리·손상 경계·할당 실패를 검사하며, XML 문서 의미 검증이나 공개 JS API 검사는 아닙니다. 범위와 실측은 [HWPX ZIP 컨테이너](hwpx-zip-container.md)를 참조합니다.
