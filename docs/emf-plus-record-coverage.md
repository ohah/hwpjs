...
`wire_validated`는 해당 record의 wire payload·참조·stream 상태 검사를 뜻할 뿐 graphics 동작 재생을 뜻하지 않습니다. [GetDC 뒤 일반 EMF record 구간](emf-plus-get-dc-interleaving.md)은 framing에서 관측·집계하지만 실제 playback surface에 적용하지 않습니다. world/page transform과 [보수적 clip state](emf-plus-clip-state.md)의 snapshot은 별도 계층에서 일부 재생하지만 property state, 일반 clip geometry, drawing rasterization, text shaping, image resampling과 저장은 후속 범위입니다.
