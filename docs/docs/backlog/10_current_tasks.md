...
### CLI (Command Line Interface) 기능 추가

- **현재 상태**: CLI 도구가 없어서 HWP 파일을 변환하려면 프로그래밍 코드를 작성해야 함
- **목적**: 
  - 명령줄에서 직접 HWP 파일을 변환할 수 있는 도구 제공
  - 배치 처리 및 자동화 스크립트에서 사용 가능
  - 개발자 및 사용자 편의성 향상
- **기능 요구사항**:
  1. **기본 변환 명령어**:
     - `hwpjs to-json <input.hwp> [output.json]` - HWP를 JSON으로 변환
     - `hwpjs to-markdown <input.hwp> [output.md]` - HWP를 마크다운으로 변환
     - `hwpjs info <input.hwp>` - HWP 파일 정보 출력 (헤더, 페이지 수 등)
  2. **이미지 추출**:
     - `hwpjs extract-images <input.hwp> [output-dir]` - 이미지 추출
     - `--format <jpg|png|bmp>` - 이미지 형식 지정
  3. **옵션 지원**:
     - `--pretty` - JSON 출력 시 포맷팅
     - `--output-dir <dir>` - 출력 디렉토리 지정
     - `--include-images` - 마크다운 변환 시 이미지 포함 여부
     - `--use-html` - HTML 태그 사용 여부
  4. **배치 처리**:
     - `hwpjs batch <input-dir> [output-dir]` - 디렉토리 내 모든 HWP 파일 변환
     - `--recursive` - 하위 디렉토리 포함
     - `--format <json|markdown>` - 변환 형식 지정
- **구현 방법**:
  - **CLI 라이브러리**: `commander` 사용 (TypeScript/JavaScript)
    - 널리 사용되는 라이브러리로 간단하고 직관적인 API 제공
    - 프로젝트에서 이미 사용 중 (crabygen, React Native 등)
  - **진입점**: `packages/hwpjs/bin/hwpjs.js` (TypeScript로 작성 후 컴파일)
  - **기능 구현**: 기존 `@ohah/hwpjs` 패키지의 NAPI 함수들을 직접 호출
  - **package.json 설정**:
    ```json
    {
      "bin": {
...
- **구조 예시**:
  ...
  packages/hwpjs/
  ├── bin/
  │   └── hwpjs.js          # CLI 진입점 (TypeScript 컴파일 결과)
  ├── src/
  │   └── cli/
  │       ├── index.ts      # CLI 메인 로직
  │       ├── commands/     # 명령어 모듈
  ...
  │       │   └── info.ts
  │       └── utils.ts      # 유틸리티 함수
  └── package.json           # bin 필드 추가
  ```
- **CLI 구현 예시** (`src/cli/index.ts`):
  ```typescript
  #!/usr/bin/env node
  
  import { Command } from 'commander';
  import { readFileSync, writeFileSync } from 'fs';
  import { toJson, toMarkdown } from '../index'; // 기존 NAPI 함수 사용
  ...
  program
    .command('to-json')
    .description('Convert HWP to JSON')
    .argument('<input>', 'Input HWP file')
    .option('-o, --output <file>', 'Output JSON file')
    .option('--pretty', 'Pretty print JSON')
    .action(async (input, options) => {
      const data = readFileSync(input);
      const result = await toJson(Array.from(data));
  ...
  program
    .command('to-markdown')
    .description('Convert HWP to Markdown')
    .argument('<input>', 'Input HWP file')
    .option('-o, --output <file>', 'Output Markdown file')
    .option('--include-images', 'Include images')
    .action(async (input, options) => {
      const data = readFileSync(input);
      const result = await toMarkdown(Array.from(data), {
...
- **사용 예시**:
  ...
  # JSON 변환
  hwpjs to-json document.hwp output.json
  
  # 마크다운 변환
  hwpjs to-markdown document.hwp --include-images
  
  # 이미지 추출
  hwpjs extract-images document.hwp ./images --format jpg
  
  # 파일 정보 확인
  hwpjs info document.hwp
  
  # 배치 변환
  hwpjs batch ./documents ./output --format markdown --recursive
  ```
- **배포 방법**:
  - 메인 npm 패키지 `@ohah/hwpjs`에 포함
  - `package.json`의 `bin` 필드로 `hwpjs` 명령어 등록
  - 전역 설치 시 `hwpjs` 명령어 사용 가능
  - `npx @ohah/hwpjs` 형태로도 사용 가능
- **의존성**:
  - 기존 `@ohah/hwpjs` 패키지의 함수들 재사용
  - CLI 라이브러리: `commander` (devDependencies)
- **영향 범위**:
  - `packages/hwpjs/package.json` (bin 필드 추가)
  - `packages/hwpjs/bin/hwpjs.js` (새 파일)
  - `packages/hwpjs/src/cli/` (새 디렉토리)
  - 문서: `docs/docs/guide/cli.md` 추가
  - 예제: CLI 사용 예시 추가
- **우선순위**: 중간
- **상태**: 계획됨
