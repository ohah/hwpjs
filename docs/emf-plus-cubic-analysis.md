...
`src/image/emf/emf_plus_cubic_flatness.zig`는 tolerance를 받지 않고 start→end의 유한 chord segment에 대한 두 control point의 최대 거리 제곱만 반환합니다. 비교 가능한 metric이어야 하므로 네 점의 모든 좌표가 finite여야 하며, 아니면 `InvalidEmfPlusCubicCoordinate`입니다. f32 좌표는 차감 전에 f64로 넓혀 유한한 극단 좌표의 차이와 제곱이 f32에서 overflow하지 않게 합니다.

start와 end가 같은 degenerate chord에는 방향이 없으므로 두 control point에서 그 한 점까지의 거리 제곱 중 큰 값을 반환합니다. 일반 chord는 projection이 구간 안이면 `cross(chord, control-start)^2 / length(chord)^2`, 앞이나 뒤면 가까운 endpoint까지의 거리 제곱을 사용합니다. 따라서 공선 control이 endpoint 바깥에 있는 overshoot도 0으로 축약하지 않습니다. 제곱근이나 임의 epsilon을 적용하지 않습니다.
...
이 계층은 접선 벡터와 tolerance 독립 metric까지만 제공하며 [adaptive flattening 계층](emf-plus-cubic-flattening.md)이 tolerance·종료·한도를 소유합니다. tangent 정규화, normal·curvature, 길이·hit testing, stroke·fill·rasterization과 저장은 후속 책임입니다. metric이 tolerance 이하라는 사실만으로 실제 렌더러의 픽셀 오차나 한컴 출력 동등성을 주장하지 않습니다.

## 검증 기록

비대칭 cubic의 양 endpoint와 `t=1/4` 미분을 독립 Bernstein 결과와 대조하고, stationary endpoint·모든 parameter 경계·내부 비유한 좌표 산술을 검사합니다. 양 endpoint는 필요한 edge가 유한하고 나머지 두 점이 NaN·±Inf인 경우에도 독립 기대 벡터를 유지하는지 확인합니다. flatness는 비대칭 수평 chord, 역방향 동일성, 구간 안 일직선 0, 공선 overshoot, 두 control 역할이 각각 더 먼 degenerate chord, f32 최대 유한 좌표의 f64 계산과 네 점·두 축의 모든 위치에서 NaN·±Inf 거부를 검사합니다. DrawBeziers와 Path 공개 adapter 결과도 canonical 계산과 대조하며, Path에는 기존 공선 fixture 외에 비공선 control fixture를 둡니다.
