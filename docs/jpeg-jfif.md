...
할당하지 않으며 썸네일 뷰를 쓰는 동안 입력을 불변·유효하게 유지해야 합니다. JFXX 확장과 APP0 위치·중복·연속성, 단일 샘플 색 변환은 위의 별도 모듈이 소유합니다. [성분 업샘플링](jpeg-upsampling.md)은 좌표와 보간 책임을 분리합니다. 명시적 unmanaged JFIF 파일 연결은 [RGB 샘플 조립](jpeg-rgb.md)이 소유하며 일반 Exif/Adobe/ICC 자동 선택·색 관리는 아직 별도입니다. 제품 HWP 이미지 지원 상태도 아직 바꾸지 않습니다.
