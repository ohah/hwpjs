...
2026-09-27 재검증: 한컴 고정 버전의 목록 `cnt`·`name`과 여덟 자식, `fieldBegin`의 직접 `parameters` 정의를 현재 검사기와 대조했습니다. 집중 필터는 Debug·ReleaseSafe·ReleaseFast 각각 11/11, 세 실파일 독립 해시·known 연결 및 `fieldBegin` 양성 known 연결은 각각 1/1, 독립 오라클의 일반·`-O` 자체검사는 각각 통과했습니다. 전체 파일별 대조는 허용 476개·ZIP 거부 6개·암호화 2개, 양성 108개 파일·루트 877개·노드 3,125개·스칼라 값 103,947바이트·미등록 자식/`cnt` 불일치 0개로 다시 일치했습니다. 별도 ZIP/XML 집계에서 `fieldBegin/parameters` 874개와 그 직접 `stringParam` 1,830개·`integerParam` 400개·`booleanParam` 12개, 나머지 `parameterset`의 그림 2개·컨테이너 1개를 확인했습니다. 앞서 같은 제품 코드에서 통과한 known-inspections 8개 shard는 이번에 다시 실행하지 않았습니다. 스칼라 타입별 의미·편집·저장은 이 결과로 검증되지 않습니다.
