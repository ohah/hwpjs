...
2026-09-27 재검증: 한컴 고정 버전 모델의 `colPr` 속성·직접 자식과 `colLine`·`colSz` 필드를 현 Zig 파서에 대조했습니다. 전용 필터는 Debug·ReleaseSafe·ReleaseFast에서 각각 8/8, 실파일 단독/known 연결은 1/1, 독립 오라클의 일반·`-O` 자체검사는 각각 통과했습니다. 전체 파일별 대조는 허용 476개·ZIP 거부 6개·암호화 2개, `colPr` 2,829개·인식 자식 169개·미지 `type` 88개·`sameSz=false` 57개·개수 불일치 0개·균일 단의 `colSz` 1개로 다시 일치했습니다. ReleaseSafe 제품 빌드도 통과했습니다. 앞서 같은 제품 코드에서 통과한 known-inspections 8개 shard는 이번 문서 전용 재검증에서 다시 실행하지 않았으며, 단 조판·편집·저장은 검증 범위 밖입니다.
