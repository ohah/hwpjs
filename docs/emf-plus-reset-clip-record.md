...
반환값은 reset clip wire 명령만 표현합니다. 현재 world-space clipping region을 실제 infinity로 변경하거나 graphics state snapshot 및 렌더링에 적용하는 기능은 구현하지 않았습니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
...
- clipping region의 infinity reset과 graphics state replay를 wire parser 완료 범위로 과장하지 않는지 확인했습니다.
