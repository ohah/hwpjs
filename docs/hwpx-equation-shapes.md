...
`shapeComment`의 직접 텍스트는 [수식 shapeComment](hwpx-equation-comments.md)가 별도로 소유합니다. 수식 자식 보고서의 `parameterset`·`metaTag` 자체는 종류·원문·속성 수·직접 하위 요소 수만 보존합니다. 별도 [공통 section 파라미터 트리](hwpx-parameter-lists.md) API가 `parameterset` 내부를, [section metaTag 텍스트](hwpx-meta-tags.md) API가 `metaTag` 직접 문자열을 관측합니다. 수식 보고서에 같은 값을 복제하지 않습니다. 다른 namespace의 동명 자식·속성은 알려진 필드로 해석하지 않습니다. 알려진 자식 안에 중첩된 요소도 해당 부모 원문에 남깁니다.
...
2026-09-27 재검증: 한컴 고정 버전의 수식 공통 도형 자식과 `sz`·`pos`·`outMargin`·`caption` 속성을 현재 공유 필드/자식 판정에 대조했습니다. `HWPX equation` 필터는 Debug·ReleaseSafe·ReleaseFast 각각 24/24, 표 도형 회귀는 Debug 5/5, 독립 수식 오라클의 일반·`-O` 자체검사는 각각 통과했습니다. 허용 476개 파일별 대조의 도형 자식 69,819개가 일치했고, 별도 ZIP/XML 인벤토리에서도 `sz`·`pos`·`outMargin` 각 23,236개와 세 종류의 속성값 합계 464,720개, 미등록 직접 자식 0개를 확인했습니다. caption·`parameterset`·`metaTag`의 수식 실파일 양성은 없으므로 해당 필드 값은 합성 검사 근거입니다. 앞서 같은 제품 코드에서 통과한 known-inspections 8개 shard는 이번에 다시 실행하지 않았습니다. 도형 배치·렌더링은 검증 범위 밖입니다.
