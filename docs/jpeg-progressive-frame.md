...
이는 8/12비트 양자화 계수 단계입니다. Progressive 샘플 평면·IDCT/RGB 연결, HWP BinData JPEG 검사 연결, 제품 HWP JS API는 아직 미완료입니다. 산술·lossless·hierarchical 처리나 실제 한글 화면과의 픽셀 동일성을 주장하지 않습니다. 제품 JS API는 CFB-only 그대로입니다.
