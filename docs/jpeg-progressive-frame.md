...
이는 8/12비트 양자화 계수 단계입니다. 후속 샘플·IDCT 연결은 [샘플 평면 계약](jpeg-progressive-samples.md)이 소유합니다. Progressive RGB 연결, HWP BinData JPEG 검사 연결, 제품 HWP JS API는 아직 미완료입니다. 산술·lossless·hierarchical 처리나 실제 한글 화면과의 픽셀 동일성을 주장하지 않습니다. 제품 JS API는 CFB-only 그대로입니다.
