...
현재 이 계층은 헤더와 저장 경계를 검사하며, [비압축 픽셀](bmp-pixels.md)은 별도 디코더가 담당합니다. [RLE4/8 색인 복호화](bmp-rle.md)는 별도 계층입니다. 내장 JPEG/PNG의 header/저장 바이트를 읽는 것과 그 압축 내용을 복호화하는 것도 다릅니다. 후속 [HWP BinData BMP 검사](hwp5-bin-data-bmp.md)는 별도 선택 계약으로 연결합니다.
