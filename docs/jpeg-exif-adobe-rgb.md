...
[ITU-T T.872 6.5.3](https://www.itu.int/rec/T-REC-T.872-201206-I/en)은 Adobe APP14 transform 1을 3성분 YCbCr로 정의합니다. 이 경로는 3성분 transform 1 또는 1성분 grayscale+transform 0과 [명시적 4성분 transform 0/2](jpeg-adobe-four-component.md)를 받으며, 모든 Adobe 헤더의 식별자·transform을 검사합니다. Adobe 부재·충돌·다른 색 선언은 오류로 남깁니다. 1/3성분 SOF ID는 공통 `jfif.classifyFrame`의 기본 `(1)`/`(1,2,3)` 검사를 재사용하고, 앞 단계의 [관측 `(0,1,2)` 예외](jpeg-component-id-compatibility.md)는 별도 옵션이 함께 켜졌을 때만 허용합니다. JFIF 선두 파일은 이 옵션과 무관하게 기존 경로를 사용합니다.

공통 JPEG 구조·엔트로피·순차/progressive 샘플·RGB 조립·ICC 조각·자원 한도를 재사용합니다. `pixel_inspection.Evidence.exif_adobe_colour`와 HWPX `Target.jpeg_exif_adobe_colour`는 **성공한** Exif+Adobe 색 경로에서만 true입니다. RGB는 unmanaged이며 Exif 방향 적용, ICC/CMM, Adobe flags, Exif TIFF 트리, 편집·저장 및 HWPX 문서 전체 의미 해석은 후속 책임입니다. 오류는 기존 이미지 대상별 `inspection_error`, ZIP·예산·OOM은 호출 오류 경계를 유지합니다.

## 실측과 적대적 검증

독립 Python ZIP/마커/Pillow 조사에서 두 로컬 corpus의 Exif 선두 JPEG는 34개입니다. 현재 33개가 선택 경로에서 해제됐고 RGB 길이 합계는 171,143,412바이트입니다. 남은 1개는 Adobe 색 선언이 없습니다. 4성분 2개의 색 계약과 픽셀 독립 대조는 [별도 문서](jpeg-adobe-four-component.md)가 소유합니다. 해제된 이미지 중 1개는 SOF ID `(0,1,2)`라 두 옵션을 모두 요구합니다. 이 개수는 픽셀 바이트 동치가 아니라 후보·형식·길이 대조입니다. 기본 엄격 JFIF corpus 집계는 변경하지 않습니다.
