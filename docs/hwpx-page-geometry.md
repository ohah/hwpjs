...
해석 대상은 `pagePr@landscape` (`WIDELY`/`NARROWLY`), `@width`, `@height`, `@gutterType` (`LEFT_ONLY`/`LEFT_RIGHT`/`TOP_BOTTOM`) 및 직접 `hp:margin`의 `header`, `footer`, `gutter`, `left`, `right`, `top`, `bottom`입니다. 숫자 어휘와 u32 한도는 공통 `xml_values.unsigned32`가 소유합니다. 이름/enum 값은 [한컴 공개 OWPML 모델의 pagePr](https://github.com/hancom-io/hwpx-owpml-model/blob/main/OWPML/Class/Para/pagePr.cpp), [margin](https://github.com/hancom-io/hwpx-owpml-model/blob/main/OWPML/Class/Para/pmargin.cpp)과 [공통 네 변](https://github.com/hancom-io/hwpx-owpml-model/blob/main/OWPML/Class/Core/marginAtt.h), [enum 목록](https://github.com/hancom-io/hwpx-owpml-model/blob/main/OWPML/Class/enumdef.h)을 기준으로 했습니다. 공개 모델의 생성자 기본값은 파일에 빠진 속성의 원값이 아니므로 주입하지 않습니다.
