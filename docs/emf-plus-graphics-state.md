# EMF+ graphics state와 world transform 재생
...
tracked stream은 Set/Reset/Multiply/Translate/Scale/RotateWorldTransform을 현재 행렬에 적용하고 `Report.world_transform`에 성공한 comment 이후 값을 노출합니다. allocation-free `State.consume`은 구조 조사 API이므로 이 값은 `null`이며 실제 EMF 검증은 tracked framing을 사용합니다.
...
- stack 테스트는 혼합 Save/Container와 target 이후 제거가 저장 시점의 world transform을 복원하는지 검사합니다.
