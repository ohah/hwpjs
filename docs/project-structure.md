...
2026-09-27 구조 인덱스 재검증에서는 이 문서의 로컬 문서 링크와 명시된 `src/`·`tools/`·`tests/`·`js/` 경로 및 축약 Zig 파일명이 현재 작업 트리에 존재하는지 확인했습니다. `build.zig`의 테스트·WASM·JS 비교 단계, Zig 0.16.0, 루트 README와 실제 `js/`의 CFB 공개 경계를 대조했습니다. ReleaseSafe 제품 빌드와 JS/WASM 비교 47개 검사는 통과했습니다. 전체 Debug Zig 테스트는 `inspectKnown` 할당 실패 반복 검사 중 장시간 실행되어 중단했으므로 통과 근거로 세지 않습니다. 이 인덱스의 링크나 소유권 안내가 연결된 모든 상세 문서의 기능 완성·실파일 검증을 뜻하지는 않습니다.
