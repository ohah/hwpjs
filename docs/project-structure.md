...
- `src/text/`: [BCP 47 문법·중복 검사](bcp47-syntax.md), [IANA 등록 검증](bcp47-registry.md). [PNG iTXt](png-international-text.md)가 공통 언어 검사를 재사용합니다.
- `src/image/`: [PNG 청크 구조·CRC 검사](png-structure.md), [행 필터 복원](png-filters.md), [IDAT 이미지 데이터 검증](png-pixels.md), [tRNS 투명도](png-transparency.md), [배경색·히스토그램](png-palette-metadata.md), [물리적 크기·유효 비트](png-sample-metadata.md), [수정 시각](png-timestamp.md), [비압축 텍스트](png-text.md), [압축 텍스트](png-compressed-text.md), [국제 텍스트](png-international-text.md), [추천 팔레트](png-suggested-palettes.md). RGBA 변환·다른 이미지 형식·HWP 컨테이너 연결은 후속 단계입니다.
