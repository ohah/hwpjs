...
| 좌표 폭별 checked extent·공통 배열 배치 | `poly_layout.zig`, [후행 호환성](emf-poly-record-compatibility.md) |
...
- 고정부의 모든 truncation, count 대비 1바이트 부족, 선언 Size 불일치와 u32 최대 count의 checked extent를 거부하고 4바이트 후행 data를 의미 배열에서 제외한다.
