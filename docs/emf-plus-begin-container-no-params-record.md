...
실제 graphics-state snapshot 생성과 재생은 구현하지 않았습니다. [EndContainer](emf-plus-end-container-record.md)는 같은 stack에서 이 레코드를 정상 종료하며 wire 수명주기를 검증합니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
