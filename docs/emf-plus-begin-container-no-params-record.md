...
실제 graphics-state snapshot 생성과 재생은 구현하지 않았습니다. EndContainer parser가 아직 없으므로 BeginContainerNoParams를 본래 방식으로 닫는 정상 쌍은 다음 파트까지 지원 완료로 세지 않고 tracked framing에서 EndContainer를 명시적으로 거부합니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
