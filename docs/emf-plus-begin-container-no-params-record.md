...
[world/page transform과 보수적 clip snapshot](emf-plus-graphics-state.md)의 생성과 EndContainer 복원은 구현했습니다. quality 등 나머지 graphics 속성의 snapshot과 렌더링은 구현하지 않았습니다. [EndContainer](emf-plus-end-container-record.md)는 같은 stack에서 이 레코드를 정상 종료하며 wire 수명주기를 검증합니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
