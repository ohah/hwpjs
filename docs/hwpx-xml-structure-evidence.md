...
이 문서는 [header·spine 구조 계층](hwpx-document-structure.md)의 선행 읽기 전용 증거입니다. `src/hwpx_structure_survey.zig`는 명시적으로 실행하는 조사 테스트이며 암호 해독을 구현하지 않습니다. 재현 명령은 [개발·검증 명령](development-commands.md)이 소유합니다.
...
선행 조사 당시 분리된 명령 `zig test src/hwpx_structure_survey.zig -O ReleaseFast`에서 위 전수 검사와 두 암호 표본 재현을 포함한 20/20 테스트가 통과했습니다. 이후 제품 header·spine 구조 조사가 추가됐으며 최신 corpus 실측은 [구조 검증 문서](hwpx-document-structure.md)가 소유합니다. 이 조사는 정규 `audit`에 포함되지 않으며 더 큰 외부 corpus나 후속 스키마 의미 검증을 대신하지 않습니다.

header의 선언 `secCnt`와 실제 section 파일 수는 한 표본에서 각각 1과 2로 달랐습니다. 사전 조사 수치로 확인한 불일치를 임의로 보정하거나 하나의 section을 버리지 않습니다. [제품 구조 계층](hwpx-document-structure.md)은 선언값·실제 관계·spine 순서를 분리해 보고합니다. header의 내부 리소스 참조·section의 문단/그림/표 의미는 이 조사에서 검증하지 않았습니다.
