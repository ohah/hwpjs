...
`src/image/emf/emf_plus_save.zig`는 MS-EMFPLUS 2.3.7.5의 EmfPlusSave wire record를 소유합니다. Type은 `0x4025`, Size는 정확히 16, DataSize와 실제 data slice는 4여야 하며 StackIndex는 little-endian u32 전 범위를 보존합니다.
...
명세는 현재 graphics state를 StackIndex와 함께 stack에 push하도록 요구합니다. 이번 파트는 StackIndex를 손실 없이 읽지만 실제 state snapshot·stack push·후속 Restore 매칭은 구현하지 않았습니다. 이 관계는 가변 stack 소유권과 원자적 rollback이 필요하므로 다음 Restore 계층의 완료 조건으로 남깁니다. 따라서 Save 재생 완료나 graphics-state 복원을 주장하지 않습니다.
