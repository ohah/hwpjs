...
후속 [Restore와 graphics-state stack](emf-plus-restore-record.md)은 Save entry에 현재 world/page transform과 [보수적 clip state](emf-plus-clip-state.md)를 저장하고 Restore target과 그 이후 entry를 함께 제거하면서 저장 시점 상태를 복원합니다. [BeginContainer](emf-plus-begin-container-record.md)와 [BeginContainerNoParams](emf-plus-begin-container-no-params-record.md)도 같은 stack에 연결되어 Restore가 뒤따른 Container를 함께 제거합니다. comment 단위 원자적 rollback과 EOF의 unmatched entry 거부도 tracked framing에 연결했습니다. quality 등 나머지 graphics 속성과 렌더링은 아직 구현하지 않았습니다.
