...
후속 [Restore와 graphics-state stack](emf-plus-restore-record.md)은 Save entry를 allocator-backed 공통 stack에 push하고 Restore target과 그 이후 entry를 함께 제거합니다. comment 단위 원자적 rollback과 EOF의 unmatched entry 거부도 tracked framing에 연결했습니다. 실제 graphics 속성 snapshot 값과 적용·렌더링은 아직 구현하지 않았고 Container record는 같은 stack의 전용 parser가 생길 때까지 명시적으로 거부합니다.
