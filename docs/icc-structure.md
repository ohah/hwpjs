...
진행 중인 `src/image/icc/`는 PNG·압축 계층에 의존하지 않습니다. 현재 헤더 필드 해석, 전체 버퍼의 선언 길이 검사, v4 프로파일 ID 검사와 [태그 테이블·저장 배치](icc-tag-table.md)를 구현했습니다. 헤더 값의 의미·등록 여부, 개별 태그 내용, 필수 태그, 색상 변환은 아직 검사하지 않습니다. 각 함수의 성공을 유효한 ICC 프로파일 인증으로 읽으면 안 됩니다.
