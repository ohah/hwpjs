...
이 계층은 DrawLines/FillPolygon의 선분 endpoint를 device 좌표로 만드는 범위입니다. clip geometry, Pen 폭·cap·join·dash, Brush fill rule·sampling, anti-aliasing, pixel offset, rasterization과 저장은 미구현입니다. Bézier·cardinal spline·Path는 각 command 의미를 보존하는 별도 device geometry 계층이 필요하며 polyline으로 강제 변환하지 않습니다. SetTSGraphics의 별도 WorldToDevice도 일반 mapper에 병합하지 않습니다. 로컬 지원 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 픽셀 출력 동등성을 주장하지 않습니다.
