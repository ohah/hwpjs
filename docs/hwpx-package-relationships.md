...
남은 작업은 `version.xml` 버전·호환성, `Contents/header.xml`과 section 순서·실제 XML 노드/참조·BinData 리소스 연결, HWP5 공통 문서 모델, 편집·쓰기·공개 WASM/JS API입니다. manifest 관계 검증 통과를 전체 HWPX 문서 검증 완료로 표시하지 않습니다.
