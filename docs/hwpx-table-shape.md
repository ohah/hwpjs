...
별도 [section 파라미터 트리](hwpx-parameter-lists.md)와 [section metaTag 텍스트](hwpx-meta-tags.md) API는 표를 포함한 section 전체의 해당 원값을 검사합니다. 이 사실이 표 보고서의 내부 필드·본문 의미 지원으로 자동 승격되지는 않습니다.
