...
반환값은 page-to-device scale wire 명령만 표현합니다. PageUnit과 PageScale을 좌표에 실제 적용하거나 graphics state snapshot 및 렌더링에 반영하는 기능은 구현하지 않았습니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
...
- PageUnit/PageScale의 실제 좌표 변환과 graphics state replay를 wire parser 완료 범위로 과장하지 않는지 대조했습니다.
