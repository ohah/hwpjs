...
기존 바이너리 형식은 바이트 시그니처를 먼저 판별합니다. PNG는 공통 PNG 픽셀/스캔라인 검사, JPEG는 기본 마커·엔트로피 **경계** 검사와 [명시적 선택 시 JFIF RGB 복호화](hwpx-jpeg-pixels.md), BMP는 공통 BMP 파일·DIB 구조와 [RGBA 픽셀 복호화](hwpx-bmp-pixels.md), GIF는 공통 GIF 프레임 인덱스 복호화를 적용합니다. WMF·TIFF·PCX로 판별된 바이트의 framing·구조·RLE 경계는 [공통 이미지 검사 계약](hwpx-picture-image-payloads.md)이 소유합니다. [SVG XML 구조](hwpx-svg-image-payloads.md)는 그 뒤에 서명 모양·선언 MIME·`.svg` 경로를 후보로 사용하되 루트 namespace를 별도로 검증합니다. 각 `Target.inspection`이 시도한 깊이를 명시하고 `inspection_error`는 실제 실패 이유를 별도로 보존합니다. 알 수 없는 형식은 `unknown_formats`로 남기고 유효한 이미지로 세지 않습니다. 기본 JPEG의 계수·색상 복원, 선택적 JPEG의 색 관리·Exif 방향·CMYK 변환, BMP 색 관리·화면 효과, GIF 합성/색 관리, WMF 그리기 의미·TIFF 픽셀·PCX 색상·이미지 렌더링은 아직 완료되지 않았습니다.
