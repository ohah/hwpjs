...
2026-09-27 현재 소스의 쪽 테두리 ID 참조 테스트를 Debug·ReleaseSafe·ReleaseFast에서 통과했습니다. 독립 ZIP/XML 조사기는 수용 476개 문서에서 해결 1,585건·대상 누락 30건을 다시 집계했고, 누락 30건의 ID 0은 한 표본의 10개 구역에서 직접 확인했습니다. 자체 반례의 Python `assert`를 명시적 예외로 바꿔 일반·`-O` 및 고장 주입으로 확인했습니다. 직전 묶음에서 제품 코드가 같은 상태로 통과한 known 8개 shard를 재사용했으며 이번에는 재실행하지 않았습니다. 0이 sentinel이라는 의미 판정이나 화면 결과는 여전히 범위 밖입니다.
