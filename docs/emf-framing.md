...
`기본 점`과 `기본 도형`은 `record_extent.zig`에서 필수 prefix·선언/실제 길이 일치·미정의 끝 extra data 수용을 공유한다. 이전에 구현한 다른 고정 길이 EMF 파서는 아직 정확 길이를 요구하므로, 상위 호환성 규칙 적용 여부를 별도로 전수 감사해야 한다.
