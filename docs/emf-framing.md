...
이 파트는 각 레코드의 wire 계약을 framing에 연결한다. handle이 Header Handles 범위에 있는지, CREATE가 빈 slot을 쓰는지, SELECT/SET/RESIZE가 살아 있는 LogPalette를 가리키는지, Start+count가 현재 palette 크기를 넘는지는 [EMF Object Table](https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-emf/e4fa4e63-9096-4cdc-b776-85e2a1e4e1f4) 상태가 필요한 후속 파트이며 아직 완료로 세지 않는다.
