...
후속 전체 Axis 조사에서는 ValueBlock 69개의 앞선 원시 u32가 0인 경우 34개, 65536인 경우 35개였습니다. 첫 차트에서도 두 번째·세 번째 값 블록에 65536이 반복됩니다. 첫 Axis 34개만 보아 항상 0이라고 검사하거나, 비영 값만 전역 객체 ID로 등록하는 규칙을 만들 수 없습니다. 원시 word를 보존하며 ID/필드 의미는 별도 검증 대상입니다. `node tests/hwp5/chart-axis-survey.mjs`의 각 value.prefix.headerWord로 재현합니다.
