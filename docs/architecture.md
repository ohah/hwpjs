...
이미지/XML 외부에서도 재사용할 수 있는 언어 태그 규칙은 `src/text/`에 둡니다. [BCP 47 문법 검사](bcp47-syntax.md)와 [IANA 등록 검증](bcp47-registry.md)은 다른 단계이며, 현재 문법 성공을 iTXt 전체 검증 성공으로 취급하지 않습니다.
