...
[HWPX 수식 원문·script 검사](hwpx-equations.md)는 소유 section 트리에서 run 직접 수식만 선택합니다. [수식 도형 자식](hwpx-equation-shapes.md)은 부모 원문 안의 부분 범위를 공유하고 속성 복제만 예산에 추가합니다. [caption 목록](hwpx-equation-captions.md)은 표와 같은 직접 목록·문단 순회를 재사용하며 속성 소유 결과만 수식 보고서에 추가합니다. 수식 문법·조판은 별도 후속 책임으로 둡니다.
