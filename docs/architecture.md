...
[masterpage 파트·section 참조](hwpx-master-pages.md)는 정규 manifest 파트의 루트 속성과 `hp:masterPage/@idRef` 연결을 분리해 확인합니다. 직접 자식 `hp:subList`의 원값·직접 문단 경계는 [ParaListType 속성](hwpx-para-list.md)이 소유하며, 문단 내부 의미와 페이지 적용 규칙은 후속 계층에 남깁니다.
