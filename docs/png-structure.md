...
보고서는 header, 전체 청크 수, IDAT 청크/바이트 수, palette 항목 수, ancillary 미검사 청크/바이트 수, 예약 비트 청크 수와 픽셀 검사 여부를 제공합니다. IEND 뒤의 추가 바이트·또 다른 PNG·추가 IEND는 **기본 정책에서** 허용하지 않습니다. 명시적 `Options.post_iend = .{ .zero_padding = max_bytes }`만 전부 0인 바깥 꼬리를 한도 안에서 검사·계수하고, 비적합 바이트 수를 `post_iend_zero_bytes`로 반환합니다. 규칙과 HWP5 선택 근거는 [IEND 뒤 0 패딩](hwp5-png-post-iend.md)이 소유합니다. 개별 빈 IDAT는 청크 계층에서 허용하지만 합쳐진 압축 스트림의 유효성은 후속 검사입니다.
