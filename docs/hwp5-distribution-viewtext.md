...
이 디코더는 비밀번호/DRM 해제가 아니며 데이터 인증을 제공하지 않습니다. 기본 stream.requireSupported는 암호화/DRM/배포용 문서 거부를 유지합니다. 명시적으로 선택하는 배포용 컨테이너 연결과 검증 범위는 [문서 본문 선택 정책](hwp5-distribution-document-policy.md)이 소유합니다. 전용 디코더 단독 성공을 문서 전체 검증 성공으로 해석하지 않습니다.
