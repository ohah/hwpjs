...
Default/LowQuality/HighQuality 별칭을 해당 구체 모드로 정규화하지 않고 wire enum을 그대로 보존합니다. 실제 image scaling, resampling filter, color-space 처리, graphics state replay, Save/Restore와 저장은 미구현입니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 한컴 렌더링과의 동등성도 주장하지 않습니다.
