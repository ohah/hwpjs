...
batch 검증은 압축/비압축 DocInfo에서 순번 1·2의 서로 다른 stream을 함께 교체하고 모든 할당 실패를 검사합니다. 빈 편집, 편집 수 한도, 0·역순·중복·범위 밖 순번, 항목별 및 총 encoded 한도, 두 번째 항목 준비 실패, 서로 다른 레코드의 물리 stream 중복, 두 번째 물리 stream 누락을 모두 출력 없이 거부하고 입력 CFB가 그대로인지 확인합니다.
...
검증 대상은 `validateKnownCounts`가 정의한 알려진 리소스이며 unknown/extra payload 내부 참조의 의미까지 증명하지는 않습니다. 기존 stream 여러 개의 내용 교체는 원자적이지만 항목 추가·삭제와 DocInfo 수정, 암호화/DRM 쓰기, 외부 LINK, 서로 다른 내부 OLE를 의미 단위로 조합하는 상위 편집 세션, 공개 JS API는 별도 범위입니다.
