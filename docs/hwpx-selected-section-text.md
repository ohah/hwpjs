...
2026-09-27 현재 내용 재검증: 현재 스트리밍 스캐너의 활성 분기·원문 section 집계·비활성 XML 검증 경계와 공유 선택 정책을 다시 대조했습니다. 독립 ZIP/XML 조사기의 허용 476개 파일에서 case/default 각각의 제거량은 문단·run·`hp:t` 2/2/2개, UTF-8 173바이트, 인라인 0개였고 양성 파일은 `issue2006/1790387_prep_final_report.hwpx` 하나였습니다. 선택 텍스트 집중 테스트는 Debug·ReleaseSafe·ReleaseFast 각 5개, 원문 텍스트·스냅샷 회귀는 Debug 15개가 통과했습니다. 이전 known-inspections 8개 shard는 제품 코드가 바뀌지 않아 파일별 대조 결과만 재사용했고 이번에 재실행한 것으로 세지 않습니다. 본문 표시 순서나 차트/OLE 시각적 동치는 검증 범위가 아닙니다.
