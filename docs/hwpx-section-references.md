...
이 검사는 문단의 `id`, [별도 API의 스타일·header 내부 참조](hwpx-header-references.md), 글꼴별 ID, 중첩 개체·그림·표·BinData의 의미·참조, 조건부 XML 분기의 선택, 2021/2024 namespace 변형, 편집·저장을 아직 다루지 않습니다. 조건부 분기가 있는 경우에도 실제 선택 분기만 판정했다고 주장하지 않고, XML에 나타난 일치 namespace의 모든 문단·run을 집계합니다. 전체 문서 검증은 후속 단계입니다.
