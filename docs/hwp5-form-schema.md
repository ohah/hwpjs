...
이 함수는 **저장된 ID의 범위**만 검사합니다. `FollowContext`에 따라 그 참조가 표시 때 사용되는지, 다른 버전의 sentinel이나 상속 규칙이 있는지는 결정하지 않습니다. 기본 문서 검증에 자동 적용하지 않았으며 invalid 진단을 활성 참조 오류로 자동 승격하지 않습니다.
...
양식 속성의 모든 값 범위·기본값·표시 활성 조건·리스트 항목·이벤트 의미, 원본 프로그램의 비 BMP 길이 및 다른 버전, 문서 기본 경로와의 정책 통합, 렌더링·편집·저장은 남아 있습니다. 명시적 경로의 자료형/저장 참조 검사를 전체 양식 의미 검증으로 집계하지 않습니다.
