...
이 record의 A bit는 명세상 SHOULD 동작이며 SmoothingMode와 별도 값으로 보존합니다. 두 값을 임의로 일치시키거나 보정하지 않습니다. 실제 text/curve rasterization, 그래픽 상태 replay, Save/Restore 상호작용과 저장은 미구현입니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 한컴 출력과의 동등성도 주장하지 않습니다.
