...
- `text_content.zig`: 검증된 본문 구간의 빌린 view를 필요한 소비자에게만 XML 정규화 UTF-8로 해제합니다. CDATA는 참조를 해석하지 않습니다.
...
스택의 이름과 태그 원문은 입력을 빌립니다. 기본 `inspect`는 스택 및 임시 속성 배열만 할당하며 정상 반환·잘못된 종료 태그·할당 실패 모두 해제합니다. 선택적 텍스트 소비자는 별도 할당을 할 수 있습니다. 반환값은 통계이며 AST나 복원 가능한 문서 모델이 아닙니다.

`visit`의 선택적 `on_content` 콜백은 루트 안의 검증된 연속 CharData와 CDATA를 현재 요소 깊이와 함께 전달합니다. view는 동기 콜백 동안만 원본 바이트를 빌리며 CDATA 구분자는 제외합니다. `toUtf8`을 호출한 소비자만 해제 결과를 소유하고 해제해야 합니다. 루트 밖 공백·주석·PI는 전달하지 않습니다. 이 콜백만으로 HWPX 스키마나 OOXML Xstring 의미를 검증하지 않습니다.
