...
현재 로컬 HWP corpus에는 EMF+ signature가 없어 실제 한컴 DrawClosedCurve 표본과 렌더링 결과는 관측하지 못했습니다. 구현 범위는 wire 구조, borrowed point iteration, Object Table 참조와 stream/framing 연결입니다. 상대좌표 누적, cardinal spline 계산, 변환·재생·래스터화와 저장은 미구현입니다.
