...
이 API는 압축 해제된 BinData 값 하나의 내부 CFB만 재생성합니다. 바깥 HWP CFB의 BinData stream 선택·[BinData 인코딩 정책](hwp5-bin-data-encoding.md) 연결·DocInfo 참조 수정·파일 전체 저장은 아직 연결하지 않았습니다. 차트 Contents의 의미 편집은 별도 parser/writer가 소유하며 이 API는 replacement bytes를 해석하지 않습니다.
