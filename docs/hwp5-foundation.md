...
## 일반 선 개체 payload (2026-09-06)

공식 표 91/92를 기준으로 `shape_line.Line`을 추가했습니다. 태그는 78(HWPTAG_BEGIN+62)이며 최소 18바이트: 시작 X/Y, 끝 X/Y의 i32 네 개와 attributes u16입니다. 공통 개체/그리기 속성은 별도 레코드이므로 여기서 다시 소비하지 않습니다. 남은 바이트는 extra로 빌립니다.

rhwp `parser/control/shape.rs:parse_line_shape_data`도 일반 선에서 u16을 읽지만 boolean으로 축소합니다. 우리는 원래 16비트를 보존합니다. 같은 태그를 쓰는 $col 연결선은 좌표 뒤 배치가 다르므로 이 파서의 대상이 아닙니다. 방향 보정 플래그의 원값을 보존할 뿐 좌표를 정렬/반전하거나 선 방향을 재계산하지 않습니다.

### 구현 후 적대적 검증

1. 네이티브에서 i32 최소/최대/-1/0 좌표, 속성 0/1/2/128/256/32768/65535를 대조했습니다. 추가 바이트와 borrowed 포인터를 확인하고 필수 18개 prefix 잘림을 거부했습니다.
2. 테스트 WASM 모드 56은 typed 필드를 다시 원시 폭으로 인코딩합니다. 각 필드 바이트에 1/128/255를 주입한 54개와 최소 길이 1개를 원문과 비교했습니다. 합성 성공 55건·잘림 거부 990건 및 오류 뒤 정상 재호출입니다.
3. 첫 전체 조사에서 선 표본이 0개로 나온 것을 추적해 구현 중 태그 77(TABLE)을 지정한 오류를 수정했습니다. 태그 78을 네이티브에서 고정하고 실제 group-drawing-02.hwp 일반 선 4개를 반드시 읽는 회귀 assertion을 추가했습니다. 합성 payload 테스트만으로 태그 선택을 입증하지 않습니다.
4. 기존 536파일 조사의 진입 가능한 구역에서 부모 $lin인 태그 78 레코드 266개를 typed 재인코딩과 대조했습니다. 속성 0은 250개, 1은 16개입니다. 모두 길이 20이며 명세 필드 뒤 2바이트는 extra로 보존합니다. 이 관측만으로 속성을 u32로 확장하지 않습니다.
5. 실제 선의 필수 prefix 잘림 4,788건을 거부했습니다. 부모 $col인 연결선 32개는 별도 deferred로 집계합니다. 진입 실패 102파일/보안 제외 4파일의 선 부재나 검증 완료를 주장하지 않으며 원본 파일을 수정하지 않았습니다.
...
## 다각형 개수·좌표 배열 배치와 원문 보존 (2026-09-07)

공식 PDF 표 99는 태그 HWPTAG_BEGIN+66(82)에 INT16 개수, INT32 x[cnt], y[cnt]를 정의합니다. rhwp parse_polygon_shape_data는 signed i32 개수 뒤 XY 쌍을 읽습니다. 실제 참조 레코드 146개도 이 32비트 개수/XY 쌍 배치와 맞습니다. 파서는 specified_i16_axes/observed_i32_points를 필수 인자로 구분하며 길이·버전으로 자동 선택하지 않습니다.

`shape_polygon.Polygon`은 개수의 signed 폭·음수 검사를 소유합니다. `shape_points.Points`는 borrowed 좌표 view와 축별/XY 배열 배치를 소유하고 Reader의 커서 검사·record_array.Records·Point.read를 공유합니다. 기존 사각형도 이 배열 view에서 네 Point를 가져오도록 바꿔 같은 배치 규칙을 복제하지 않습니다. 네이티브 메모리 구조체 배열로 wire를 reinterpret하지 않습니다.

개수는 남은 바이트/8과 먼저 비교한 뒤 곱하고 슬라이스를 빌립니다. 선언 개수에 비례한 할당은 없고 잘못된 개수만큼 순회하지 않습니다. rhwp는 음수를 0으로 바꾸거나 실제 바이트 수에 맞춰 개수를 줄이지만 신규 파서는 NegativePointCount/UnexpectedEnd로 거부합니다. 정상 개수 0/1/2는 보존합니다. 실제 2점 표본 4개가 있으므로 최소 3점, 시작/끝 중복 제거, 자동 닫힘을 강제하지 않습니다.

### 실제 대조

- 참조 조사에서 다각형 146개, 총 2,120개 Point를 관측했습니다. 점 개수 범위는 2~188이고 모두 geometry 뒤 4바이트 extra가 있습니다. 이 꼬리의 의미/필수 여부는 확정하지 않았으므로 기본값으로 채우거나 무조건 4바이트를 요구하지 않습니다.
- 꼬리 raw hex는 00000000 138개, 01000000 2개, 0d001000 3개, 29002f00 3개입니다. 비영 꼬리 8개를 확인했으므로 이를 일괄 0 패딩으로 해석하거나 버리지 않습니다. 숫자/비트/추가 필드 의미는 아직 확정하지 않았습니다.
- table-vpos-01 HWP/HWPX의 2개(8점), shape-001.hwp와 hwpx/shape-001.hwpx의 2개(8점), hwp3-sample11-hwp5.hwp와 hwp3-sample11-hwpx.hwpx의 21개(161점)를 명시적으로 짝지었습니다. 총 25개 다각형의 177개 XY와 개수가 WASM/독립 HWPX XML에서 모두 일치합니다. 파일 이름만으로 동일성을 가정하지 않고 실제 필드를 대조했습니다.
- HWPX는 기존 테스트용 ZIP/XML helper로 읽었습니다. 제품 HWPX 파서·한글 프로그램 실행·렌더링 일치를 검증한 것이 아닙니다. 원본 파일은 수정하지 않았습니다.

### 구현 후 적대적 검증

1. 두 배치의 signed 최소/최대/-1/양수/음수 좌표 순서와 배열의 borrowed 주소, count 범위 밖 get(index)의 null을 검사합니다. 동일 바이트에 다른 명시적 배치를 적용하면 다른 결과가 나오는 것도 확인합니다.
2. 개수의 모든 16/32비트를 하나씩 뒤집습니다. 남은 데이터보다 큰 개수와 음수는 거부하고 유효한 작은 개수는 그 개수와 나머지 원문 꼬리로 대조합니다. 오류 뒤 정상 입력은 독립 기대값과 다시 비교합니다.
3. 네이티브 usize 최대 개수 및 비정상 Reader offset을 검사합니다. 재검토 중 새 배열 helper의 뺄셈 전에 Reader.take(0)의 커서 검사를 추가해 unsigned underflow를 막았습니다. 실패와 0점 읽기에서 커서를 보존합니다. WASM에서는 i16 최대 32,767점과 i32 배치의 32,768점도 읽어 개수를 축소하지 않는지 확인합니다.
4. 좌표·꼬리 각 바이트의 1/128/255 변이, 모든 필수 geometry prefix 잘림, 꼬리 0~4바이트 유지와 미지 바이트 보존을 검사합니다. 실제 레코드의 필수 prefix 17,544개도 거부하며 꼬리 일부만 남으면 해당 extra 길이/바이트를 그대로 보고합니다.
5. 기존 실제 사각형 2,175개 및 짝 HWPX 대조를 전체 audit에 포함해 좌표 배열 SSOT 변경의 회귀를 검사합니다. 다각형 payload의 정상 원문 대조와 전체 문서 소유권·기하 의미·시각적 일치를 혼동하지 않습니다.
...
## 곡선 점 배열·구간 종류·꼬리 보존 (2026-09-07)

공식 PDF 표 103은 태그 HWPTAG_BEGIN+67(83)의 INT16 점 개수, x[cnt]/y[cnt], BYTE[cnt-1] 구간 종류(0=line, 1=curve)를 정의합니다. 로컬 rhwp parse_curve_shape_data와 실제 8개 레코드는 i32 개수/XY 쌍을 사용합니다. 명시적 specified_i16_axes/observed_i32_points 배치를 유지하고 길이·버전으로 자동 추정하지 않습니다.

`shape_points.Points.readCounted`가 signed 개수·음수 거부·배치·경계를 소유하도록 공통화하고 다각형/곡선에서 공유합니다. 읽기 실패 시 외부 Reader 위치를 유지합니다. Curve는 그 뒤 인접 점 쌍 수만큼 구간 바이트를 빌리고 나머지를 extra로 보존합니다. 0/1점의 인접 쌍 수는 0으로 정의해 count-1 underflow를 막되 원시 개수를 늘리거나 음수 개수를 0으로 바꾸지 않습니다. 실제 0/1점 곡선 표본을 검증한 것은 아니며 이 경우의 기하 의미는 별도입니다.

rhwp는 개수를 남은 좌표 바이트에 맞춰 줄이고 구간이 부족하면 read_u8().unwrap_or(0)으로 채운 뒤 u32 꼬리를 버립니다. 신규 코어는 선언된 점/구간 길이가 부족하면 오류이며, 미지 종류 2~255와 꼬리를 그대로 보존합니다. 추가 할당 없이 borrowed view를 반환하고 단위 행 읽기는 기존 record_array/Point.read가 소유합니다.

### 실제 대조

- 참조 곡선 8개는 총 2,514점·2,506구간이며 종류 0은 54개, 1은 2,452개입니다. 모든 꼬리는 00000000 네 바이트입니다. 이것만으로 꼬리의 의미/필수 길이를 확정하지 않아 raw extra로 남깁니다. 필수 배열 길이 검증과 모든 의미적 손상 검출은 다릅니다.
- 행정업무 편람 Section4의 3점 곡선 2개, 교육 문서 여섯 변형본의 418점 곡선 각 1개입니다. 여섯 파일이 같은 계열의 변형본임을 고려해야 하며 독립된 여섯 종류의 기하 표본이라고 주장하지 않습니다.
- 짝 HWPX가 있는 행정업무 편람 Section4(2개/4구간), 교육 2022(1개/417구간), 교육 2024-구분선아래20구분선위20(1개/417구간)에서 총 4개 곡선의 838구간 양 끝 XY와 LINE/CURVE를 실제 WASM 결과와 모두 대조했습니다. 테스트용 sectionXml helper에 명시적 구역 번호를 추가해 Section0만 검사하는 위치 편향을 피합니다. 제품 HWPX 파서나 렌더링 검증은 아닙니다.

### 구현 후 적대적 검증

1. 두 배치의 signed 최소/최대/음수 좌표 순서, 구간 1/255 및 비영 꼬리를 독립 비교합니다. 구간/꼬리의 borrowed 포인터도 네이티브에서 확인합니다.
2. 점·구간 필수 바이트의 모든 prefix를 거부하고 꼬리 0~4바이트는 해당 extra 길이·원문으로 보존합니다. 실제 8개의 필수 prefix 22,650개도 거부합니다. 꼬리 의미가 미확정이므로 임의로 끝의 네 바이트를 삭제하지 않습니다.
3. 합성 0/1/2/3점, 음수/최대 signed/개수+1, i16 최대 32,767점과 i32의 32,768점 및 미지 구간 255를 검사합니다. 작은 선언 개수로 clamp하거나 불완전 구간에 가짜 0을 넣지 않습니다.
4. 각 좌표·구간·꼬리 바이트의 1/128/255 변이와 실패 뒤 정상 재호출을 독립 기대값과 대조합니다. 잘못된 배치 모드 2/255·모드 바이트 부재도 거부합니다. readCounted의 음수/좌표 잘림 실패는 외부 커서를 보존합니다.
5. XML 구역 번호 -1/분수/NaN/65536을 거부하고 기존 모든 Section0 짝 대조를 유지합니다. 공통 개수 읽기를 바꾼 다각형 146개 및 소유권·문서/CFB 검증도 전체 audit에서 재실행합니다.

현재는 곡선 payload 코어와 테스트용 WASM 모드 70입니다. 곡선의 직접 소유권·누락·중복·문서 연결, 실제 보간/닫힘/조판, 그림/연결선 등 남은 도형, 전체 문서 모델·HWPX·편집/저장은 계속 남아 있습니다. 원본 파일은 수정하지 않았습니다.

최종 Debug·ReleaseSafe·ReleaseFast audit 모두 모드별 네이티브 209/209, Node 47/47, HWP5 WASM 401,191회 검사 통과. 곡선 합성 성공 198/거부 5,357, 실제 8개·필수 prefix 거부 22,650, 짝 HWPX 4개/838구간 일치를 포함합니다. CFB 12,000회 변이 trap 0, Zig 포맷·변경 JS 문법·diff 검사 통과. 로그는 `/tmp/hwpjs-curve-{debug,safe,fast}.log`입니다.

## 곡선 직접 소유권과 문서 연결

2026-09-07. `curve_validation.zig`는 첫 ID가 $cur인 SHAPE_COMPONENT의 직접 태그 83 하나를 검사합니다. 기존 owned_record.componentChild와 Curve.parse를 조립하고 부모/개수/좌표/구간 파싱을 복제하지 않습니다. 문서 curve_layout 기본값은 observed_i32_points이며 specified_i16_axes도 명시적으로 선택할 수 있습니다. 자동 배치 fallback은 없습니다.

구역 보고서는 curves/points/segments/short_point_sets/unknown_segments/extra_bytes 여섯 항목입니다. 2점 미만과 종류 2~255는 원문 진단이지 임의 보정이나 조판 오류가 아닙니다. 테스트 WASM 모드 71은 소유권, 72/73은 배치를 선택한 문서/CFB 검사입니다. 보고서 stride는 공통 JS schema에서 624바이트로 관리합니다.

적대적 검증 범위:

1. 두 배치에서 누락/중복/고아/다른 도형 부모/손자/형제/중간 미지 레코드를 검사하고 실패 뒤 정상 입력을 다시 검사합니다. 네이티브는 정상·소유권 오류·음수 개수·좌표 뒤 구간 누락의 6경로마다 모든 할당 실패를 주입합니다.
2. 실제 교육 2022 파일의 곡선 1개(418점/417구간)에 대해 제거·복제·잘림·음수/초과 개수·고아를 단독/문서/재저장 CFB의 세 경로에서 검사합니다. 원본 파일은 수정하지 않습니다.
3. 같은 실제 본문을 독립 JS로 specified_i16_axes 배치로 변환하고 두 배치의 문서/CFB 보고서를 비교합니다. observed 재인코딩은 원본 본문과 바이트 일치하며 specified 변환은 합성 입력임을 구분합니다. 좌표가 전부 있지만 구간 전체/마지막 구간이 없는 경우도 오류가 전파됩니다.
4. 구간 종류를 255로 바꾼 진단을 문서/CFB에서 확인하고, 서로 다른 unknown_segments 값을 가진 두 구역의 입력 순서를 뒤집어 정렬 결과를 비교합니다. 실제 참조 조사에 진입한 모든 본문의 곡선 소유권도 독립 JS 부모 인덱스와 대조합니다.
5. 최초 전체 CFB 검사에서 LimitExceeded가 발생했습니다. 해당 약 5.6MB 파일의 검사 대상 decoded 총량은 87,772,864바이트이며 BinData가 87,160,034바이트입니다. Node의 독립 압축 해제/스트림 합산과 WASM 보고서를 대조하고 총량과 동일한 한도에서 성공, 총량-1과 64MiB에서 거부됨을 확인합니다. 이 fixture에만 128MiB 테스트 한도를 명시하며 제품의 64MiB 기본값이나 압축 오류 처리를 완화하지 않습니다.
