...
타입/객체 테이블의 allocator/options는 파일에서 복호화한 엔트리 필드가 아니며 이 wire에 포함하지 않습니다. 참조별 introduced·start/end 같은 중첩 반환 필드는 사전 엔트리와 별개이며 아직 전체 대조하지 않습니다. 독립 oracle이 지원하지 않는 배치까지 사전 검증을 완료한 것은 아닙니다.
