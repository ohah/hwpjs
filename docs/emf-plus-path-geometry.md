...
이 계층은 path command topology까지만 구현합니다. [Path device command 계층](emf-plus-path-device-commands.md)은 Move·Line·Bézier와 빈 figure metadata를 보존해 world/page/device 변환을 적용하고 [Path device figure geometry](emf-plus-path-device-geometry.md)가 이를 figure별로 소유·색인하며 [Path device figure polyline](emf-plus-path-device-polyline.md)이 공용 cubic flattener로 결과 point range를 조립합니다. CloseSubpath의 실제 closing edge는 별도 [Path closing segment 계층](emf-plus-path-segments.md)이, FillPath의 열린 figure 경계는 [Path fill boundary 계층](emf-plus-path-fill-segments.md)이 command를 재사용해 출력하고 [Path device segment 계층](emf-plus-path-device-segments.md)이 두 결과의 역할과 metadata를 보존해 같은 변환을 재사용합니다. [공용 cubic evaluator](emf-plus-cubic-evaluation.md)가 Bézier 점을 계산합니다. DashMode stroke 적용, marker 소비자, clip boolean geometry, fill rule, rasterization과 저장은 후속 범위입니다. PathGradient·Region·CustomLineCap이 보유한 중첩 Path는 같은 `Path.commands()`·`Path.segments()`·`Path.fillSegments()`를 호출할 수 있지만 상위 객체 의미를 여기서 추정하지 않습니다. 로컬 HWP corpus에는 EMF+ signature 표본이 0개라 실제 한컴 렌더링 동등성도 주장하지 않습니다.
