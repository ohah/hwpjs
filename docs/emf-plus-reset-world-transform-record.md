...
반환값은 reset 명령의 wire 존재만 표현합니다. graphics state의 현재 행렬을 identity로 실제 변경하거나 Save/Restore snapshot 및 렌더링에 적용하는 기능은 구현하지 않았습니다. 로컬 HWP corpus에는 EMF+ signature 표본이 없어 실제 한컴 출력 동등성도 주장하지 않습니다.
