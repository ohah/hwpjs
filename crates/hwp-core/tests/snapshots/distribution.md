...
ᄒᆞᆫ글 문서 파일 형식 – 배포용 문서

ᄒᆞᆫ글 문서 파일 형식 – 배포용 문서



![이미지](/Users/yoonhb/Documents/workspace/hwpjs/crates/hwp-core/tests/snapshots/images/distribution/BIN0001.bmp)
...
2.4. 해시코드와 AES-128 알고리즘을 이용하여 레코드 복호화	8
...
(주)한글과컴퓨터(이하 ‘한컴’)는 문서 형식의 개방성과 표준화에 대하여 적극 찬성합니다. 한컴은 ᄒᆞᆫ글 97의 문서 형식을 무상으로 지원한 바 있으며, ᄒᆞᆫ글 2002~2010 문서의 XML 형식은 HWPML에 대해서도 문서 형식을 공개한 바 있습니다. 개방형 문서 표준화 및 코드 관련 위원회에도 적극적으로 참여하여 파일 형식의 표준화와 개방성을 위해 노력해 왔습니다. 이러한 결과로 HWPML 스펙이 OWPML란 이름으로 한국산업표준(KS X 6101:2011)으로 제정되었습니다. 또한, 한컴오피스에서 기록물 장기보존 표준 포맷인 PDF/A-1의 지원과 ISO 국제 문서 형식인 ODF와 OOXML 파일 형식의 불러오기와 저장하기를 적극적으로 지원하였습니다.
...
본 문서는 ᄒᆞᆫ글 워드 프로세서의 파일 저장 형식 중, ᄒᆞᆫ글 2002 이후 제품에서 사용되는 ᄒᆞᆫ글 문서 파일 형식의 배포용 문서에 관하여 설명한다.

본 문서는 ᄒᆞᆫ글 문서 파일 형식의 배포용 문서에 관한 주요한 자료 구조에 대해서 설명한다.

ᄒᆞᆫ글 문서 파일형식 5.0, 수식, 차트, ᄒᆞᆫ글 문서 파일 형식 3.0, HWPML에 관해서는 별도의 문서에서 설명한다.
...
|   | 배포용문서<br> | 일반문서<br> |
|---|---|---|
| 본문 스트림<br> | ViewText/Section<br> | BodyText/Section<br> |
| 암호화 여부<br> | ○<br> | ×<br> |
| 비고<br> | 복사, 인쇄제한을 위한 플래그를 가짐<br> |   |
...
|   | 길이(바이트)<br> | 설명<br> |
|---|---|---|
| BYTE array[256]<br> | 256<br> | 배포용 문서 데이터<br> |
| 전체 길이<br> | 256<br> |   |
...
|   | 길이(바이트)<br> | 설명<br> |
|---|---|---|
| 본문 스트림<br> | 80<br> | - 배포용 문서를 생성할 때 사용된 암호의 SHA1 해시코드<br> - 암복호화용 키(처음부터16바이트까지)<br> |
| 암호화 여부<br> | 2<br> | 배포용 문서 옵션 flag<br> 0x01 = 복사방지 설정<br> 0x02 = 인쇄방지 설정<br> |
| 전체 길이<br> | 82<br> |   |
...
![이미지](/Users/yoonhb/Documents/workspace/hwpjs/crates/hwp-core/tests/snapshots/images/distribution/BIN0002.bmp)
...
배포용 문서 데이터에서 해시코드를 추출하기 위해서는 우선 특정패턴을 지닌 난수배열을 생성해야 한다. 이때 패턴을 생성하기 위해 MS Visual C의 랜덤함수(srand(), rand())를 사용한다.
...
2. rand()함수의 결과 값을 이용하여 배열을 채운다. 단, rand()함수가 호출되는 순번에 따라 그 사용 방식이 달라진다.  
홀수번째 : 배열에 채워지는 값  
짝수번째 : 배열에 채워지는 횟수
...
![이미지](/Users/yoonhb/Documents/workspace/hwpjs/crates/hwp-core/tests/snapshots/images/distribution/BIN0003.bmp)
...
### 4) **해시코드와 AES-128 알고리즘을 이용하여 레코드 복호화**
...
- ᄒᆞᆫ글 문서 파일 구조 파트별로 구성

- ᄒᆞᆫ글 문서 파일 형식 – 배포용 문서 형식 공개

---



| 한글 문서 파일 구조 – 배포용 문서<br> |   |
|---|---|
|   |   |
| 발행처<br> | (주) 한글과컴퓨터<br> |
| 주  소<br> | (우) 463-400<br> 경기도 성남시 분당구 대왕판교로 644번길 49 한컴타워 10층<br> 전화: (031) 627-7000<br> 팩스: (031) 627-7709<br> |
|   |   |
