        ...
        if first_row_has_content {
            ...
            while col < col_count {
                if let Some(cell_data) = &grid[0][col] {
                    ...
                    // border_fill 클래스 추가 / Add border_fill class
                    let mut class_names = Vec::new();
                    if cell_data.border_fill_id > 0 {
                        class_names.push(format!(
                            "{}border-fill-{}",
                            css_prefix, cell_data.border_fill_id
                        ));
                    }
                    if !class_names.is_empty() {
                        attrs.push(format!(r#"class="{}""#, class_names.join(" ")));
                    }
...
pub fn generate_border_fill_css(document: &HwpDocument, css_prefix: &str) -> String {
    ...
    for (idx, border_fill) in document.doc_info.border_fill.iter().enumerate() {
        ...
        for (idx, border) in border_fill.borders.iter().enumerate() {
            if border.line_type != 0 && border.width > 0 {
                let width_px = border.width as f32 * 0.75; // HWP 단위를 px로 변환 (대략적)
                let color = format_colorref(&border.color);
                css.push_str(&format!(
                    "        border-{}: {}px solid {};\n",
                    border_names[idx], width_px, color
                ));
            }
        }
        ...
        match &border_fill.fill {
            FillInfo::Solid(solid) => {
                if solid.background_color.0 != 0xFFFFFF {
                    let bg_color = format_colorref(&solid.background_color);
