...
pub fn render_page(
    ...
    footer_content_height_mm: Option<f64>,
) -> String {
    ...
    let footer_top_mm = page_def
        .map(|pd| round_to_2dp(height_mm - pd.bottom_margin.to_mm() - pd.footer_margin.to_mm()))
        .unwrap_or_else(|| round_to_2dp(height_mm - 10.0));
    ...
    if let Some(s) = header_fragment {
        if !s.is_empty() {
            ...
            let actual_page = (page_start_number as usize + page_number - 1).to_string();
            let header_html = s.replace("<!--PN-->", &actual_page);
            html.push_str(&format!(
                r#"<div class="hcD" style="left:{}mm;top:{}mm;"><div class="hcI">{}</div></div>"#,
                left_mm, header_top_mm, header_html
            ));
        }
    }
    // 꼬리말 hcD / Footer hcD
        ...
        if flags.position != PageNumberPosition::None {
            ...
            let (left_mm, top_mm) = match flags.position {
                PageNumberPosition::BottomCenter => {
                    ...
                    let left = round_to_2dp(page_width_mm / 2.0);
                    let top = page_def
                        .map(|pd| {
                            // 페이지 높이 - 아래 여백 / Page height - bottom margin
                            round_to_2dp(page_height_mm - pd.bottom_margin.to_mm())
                        })
                        .unwrap_or_else(|| round_to_2dp(page_height_mm - 10.0));
                    (left, top)
                }
            ...
            let (width_mm, height_mm) = if char_shape_id < document.doc_info.char_shapes.len() {
                if let Some(char_shape) = document.doc_info.char_shapes.get(char_shape_id) {
                    ...
                    let text_width_mm = round_to_2dp(char_count * font_size_mm * 0.6);

                    // 텍스트 높이 계산: 폰트 크기 * line-height (약 1.2) / Calculate text height: font size * line-height (approx 1.2)
                    let text_height_mm = round_to_2dp(font_size_mm * 1.2);
