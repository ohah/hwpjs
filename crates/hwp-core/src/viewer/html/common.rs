...
///
/// HTML 변환에 사용되는 공통 함수들을 제공합니다.
/// Provides common functions used in HTML conversion.
...
/// Format image HTML - either as base64 data URI or file path
/// 이미지 HTML 포맷 - base64 데이터 URI 또는 파일 경로
pub(crate) fn format_image_html(
    document: &HwpDocument,
    bindata_id: crate::types::WORD,
    base64_data: &str,
    ...
    match image_output_dir {
        Some(dir_path) => {
            // 이미지를 파일로 저장하고 파일 경로를 HTML에 포함 / Save image as file and include file path in HTML
            match save_image_to_file(document, bindata_id, base64_data, dir_path) {
                Ok(file_path) => {
                    ...
                    format!(
                        r#"<img src="images/{}" alt="이미지" class="{}image" />"#,
                        file_name, css_prefix
                    )
                }
                Err(e) => {
                    ...
                    format!(
                        r#"<img src="data:{};base64,{}" alt="이미지" class="{}image" />"#,
                        mime_type, base64_data, css_prefix
                    )
                }
            }
        }
