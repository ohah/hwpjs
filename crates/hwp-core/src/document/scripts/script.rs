...
impl Script {
    ...
    pub fn parse(data: &[u8]) -> Result<Self, String> {
        ...
        if data.len() < 20 {
            return Err(format!(
                "Script must be at least 20 bytes, got {} bytes",
                data.len()
            ));
        }
        ...
        let header = if header_len > 0 && offset + (header_len * 2) <= data.len() {
            let header_bytes = &data[offset..offset + (header_len * 2)];
        ...
        if offset + 4 > data.len() {
            return Err("Insufficient data for script source length".to_string());
        }
        ...
        let source = if source_len > 0 && offset + (source_len * 2) <= data.len() {
            let source_bytes = &data[offset..offset + (source_len * 2)];
        ...
        if offset + 4 > data.len() {
            return Err("Insufficient data for script pre source length".to_string());
        }
        ...
        let pre_source = if pre_source_len > 0 && offset + (pre_source_len * 2) <= data.len() {
            let pre_source_bytes = &data[offset..offset + (pre_source_len * 2)];
            decode_utf16le(pre_source_bytes).unwrap_or_default()
        } else {
            String::new()
        };
        offset += pre_source_len * 2;

        // DWORD 스크립트 Post 소스 길이 (4 bytes) / Script post source length (4 bytes)
        if offset + 4 > data.len() {
            return Err("Insufficient data for script post source length".to_string());
        }
        ...
        let post_source = if post_source_len > 0 && offset + (post_source_len * 2) <= data.len() {
            let post_source_bytes = &data[offset..offset + (post_source_len * 2)];
            decode_utf16le(post_source_bytes).unwrap_or_default()
        } else {
            String::new()
        };
        offset += post_source_len * 2;
