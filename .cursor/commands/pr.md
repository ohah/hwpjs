...
This repo (ohah/hwpjs) uses the **ohah** GitHub account for push and PR.
...
2. **SSH remote**: Ensure origin is `git@github.com-private:ohah/hwpjs.git` so push uses ohah's SSH key; if not, run `git remote set-url origin git@github.com-private:ohah/hwpjs.git`.
