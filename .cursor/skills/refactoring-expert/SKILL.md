...
- **작은 파일 권장**: 한 파일이 한 타입·한 관심사만 담당하면 이름만으로 역할을 추측하기 쉬워 사람·AI 모두 가독성이 좋다. `bodytext/shape_component/`처럼 타입별로 파일을 나누는 패턴을 따른다.
- **기준**: 한 파일이 **300~400줄을 넘으면** 역할별로 쪼개기를 고려한다. `mod.rs`는 `mod` 선언과 `pub use` 위주로 두고, 실제 타입·로직은 개별 파일에 둔다.
- **디렉터리로 묶기**: 한 디렉터리에 파일이 너무 많아지면(예: 20개 이상) 하위 디렉터리로 묶는다. 예: `ctrl_header/`, `shape_component/`.
- **분리 시 주의**: 한 타입의 정의와 그 타입만 쓰는 작은 로직은 한 파일에 두고, 여러 타입을 함께 다루는 큰 흐름(예: 문단 전체 처리)은 별도 파일/모듈로 둔다. 관련 코드가 지나치게 흩어지지 않도록 균형을 맞춘다.
- **과도한 분리 피하기**: 파일 개수만 늘고 각 파일에 코드가 조금씩만 있으면 오히려 탐색이 어려워진다. “이 파일이 제공하는 공개 API가 명확한가?”를 기준으로 나눈다.
