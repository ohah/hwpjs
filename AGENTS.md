...
- `shape_component.zig`는 명시적 single_id/double_id와 42바이트 구성요소 필드를 소유합니다. 동일한 인접 DWORD를 보고 ID 개수를 추정하지 않습니다. `rendering.zig`는 translation 및 scale/rotation 쌍을 소유하고 binary.record_array로 96바이트 쌍의 borrowed 접근을 공유합니다. 행렬 IEEE-754 비트는 원문 보존하며 합성·정규화하지 않습니다. 도형 종류별 꼬리·문서 연결·조판 검증은 아직 별도입니다.
