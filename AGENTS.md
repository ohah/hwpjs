...
- `shape_line.zig`는 일반 선의 태그 78 payload(표 92)를 소유합니다. 네 signed 좌표와 u16 속성 원값을 보존하며, 후반부를 속성에 합치거나 bool로 축소하지 않습니다. `line_validation`은 owned_record.find로 $lin의 직접 자식 한 개와 고아/누락/중복을 검사하며 문서/CFB에 연결됩니다. 같은 태그를 쓰는 $col은 다른 배치이므로 deferred_connectors로 남기고 일반 선으로 해석하지 않습니다. 비boolean 속성은 오류가 아닌 원값 진단입니다.
