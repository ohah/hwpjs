...
현재는 바이트 리더, CFB v3/v4 읽기·strict 검증·새 컨테이너 생성/재저장, HWP5 헤더·압축 스트림·레코드 경계 읽기 코어가 구현되어 있습니다. HWP/HWPX 문서 의미 해석·편집·저장은 미구현입니다. HWP5 코어는 테스트용 WASM에서 검증하며 제품 JS 공개 API는 아직 CFB만 제공합니다. 지원 범위는 구현·테스트로 확인하고, 예정 기능을 완료된 기능처럼 설명하지 않습니다.
...
- `src/hwp5/docinfo/`: 문서 속성·ID 매핑 payload 해석과 태그 dispatch를 분리합니다. 실제 필드 부재(null)와 값 0, 버전상 기대 슬롯 수를 구분합니다. 전체 문서 조립/참조 검증은 아직 미구현입니다.
