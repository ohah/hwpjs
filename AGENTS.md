...
- `ole.zig`는 SHAPE_COMPONENT_OLE의 spec24/observed26 payload를 명시적으로 구분합니다. 속성은 각각 u16/u32이지만 BinData ID는 두 배치 모두 u16이며 테두리 색을 ID에 섞지 않습니다. signed 크기·두께와 미지 꼬리를 보존하고 외부 링크나 임베디드 프로그램을 실행하지 않습니다. 도형 소유권·BinData 참조·문서 검증 연결은 아직 별도입니다.
