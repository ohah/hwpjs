...
- `shape_border.zig`는 명시적 spec11/observed13 선 두께 배치와 원문 꼬리를 소유합니다. `line_attributes.zig`는 표 87 비트 view의 SSOT이며 OLE의 borderAttributes도 공유합니다. 예약값·색상 상위 비트·signed 두께를 정규화하지 않습니다. 테두리 전용 파서는 아직 도형 문서 검사에 연결하지 않았습니다.
- `drawing_style.zig`는 관측 테두리→기존 Fill→종류별 alpha→그림자 배치를 조립합니다. Fill의 기본 파서를 복제하지 않습니다. `docinfo/fill_alpha.zig`는 pattern/gradient/image 순서의 선택 원시 바이트, `shadow.zig`는 16바이트 관측 블록을 소유합니다. 미지 Fill 비트가 있으면 뒤 위치를 추정하지 않고 unknown tail로 남깁니다. 이 스타일 파서는 아직 문서 검사에 연결되지 않았습니다.
