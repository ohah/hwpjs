...
- `shape_line.zig`는 일반 선의 태그 78 payload(표 92)를 소유합니다. 네 signed 좌표와 u16 속성 원값을 보존하며, 후반부를 속성에 합치거나 bool로 축소하지 않습니다. 같은 태그를 사용하는 $col 연결선은 다른 배치라 적용하지 않습니다. 현재 payload 검증이며 문서의 직접 소유권/중복/누락 검사 연결은 별도입니다.
