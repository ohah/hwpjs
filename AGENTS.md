...
- `shape_arc.zig`는 태그 81의 명시적 specified_u32(28바이트)/reference_u8(25바이트) 배치를 구분합니다. Header tagged union으로 속성/종류 필드의 원시 폭을 보존하고 세 Point와 extra를 빌립니다. 명세 표 참조의 불일치를 타원 비트 규칙으로 추정 보정하지 않습니다. 참조 corpus에서 실제 호 레코드는 아직 확보하지 못했으며 문서 소유권/조판 연결은 별도입니다.
