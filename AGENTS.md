...
- `note_control.zig`는 위 쪽 단위 모양과 별개인 fn/en 컨트롤 속성을 소유합니다. spec8은 불투명 원문, observed16은 전체 폭의 관측 필드이며 배치는 호출자가 선택합니다. 문서 검사 연결과 직접 문단 리스트 검증은 아직 없으며 짧은 입력을 기본값으로 채우지 않습니다.
