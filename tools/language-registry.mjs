...
export function catalog(subtags,extensions){
  ...
  assert.match(date,/^\d{4}-\d{2}-\d{2}$/);assert.match(extension_date,/^\d{4}-\d{2}-\d{2}$/);
  ...
  return {date,extension_date,sources,sha256:{subtags:createHash('sha256').update(subtags).digest('hex'),extensions:createHash('sha256').update(extensions).digest('hex')},record_counts:counts,...groups,extlang:Object.fromEntries(Object.entries(extlang).sort()),extensions:identifiers.join(' ')};
}
...
export function tables(c){
  assert.match(c.date,/^\d{4}-\d{2}-\d{2}$/);assert.match(c.extension_date,/^\d{4}-\d{2}-\d{2}$/);
  ...
  for(const type of Object.keys(patterns)){const values=c[type].split(' ').flatMap(expand).sort();assert.equal(new Set(values).size,values.length);for(const value of values)assert.match(value,patterns[type]);files[`${type}.txt`]=values.map(s=>s.padEnd(8,'!')).join('\n')+'\n';}
